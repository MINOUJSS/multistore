...
class SupplierOrderController extends Controller
{
    ...
    {
        $order = SupplierOrders::findOrfail($order_id);
        $order_items = SupplierOrderItems::where('order_id', $order_id)->get();
    ...
    {
        $order_item = SupplierOrderItems::findOrfail($order_item_id);
        // get order
        $order = SupplierOrders::findOrfail($order_item->order_id);
    ...
    public function edit_order($id)
    {
        ...
        // $products = SupplierProducts::pluck('name', 'id');
        $products = SupplierProducts::all();
        $wilayas = Wilaya::all();

        return view('users.suppliers.orders.edit_order', compact('order', 'products', 'wilayas'));
    }

    // update
    public function update_order(Request $request, $id)
    {
        $order = SupplierOrders::with('items')->findOrFail($id);
        ...
        if ($request->has('items')) {
            foreach ($request->items as $itemId => $data) {
                $item = SupplierOrderItems::find($itemId);
                if ($item) {
                    $item->update([
        ...
        if (!empty($deletedItemIds)) {
            SupplierOrderItems::whereIn('id', $deletedItemIds)->delete();
        }

        return response()->json(['success' => true]);

        // return redirect()->route('supplier.order.edit', $order->id)
        //                  ->with('success', 'تم تحديث الطلب بنجاح.');

        // return redirect()->route('supplier.orders.index')->with('success', 'تم تحديث الطلب بنجاح');
    }

    public function deleteItem($id)
    {
        $item = SupplierOrderItems::findOrFail($id);
        // $item->delete();
        // get order
        $order = SupplierOrders::findOrfail($item->order_id);
        if (count($order->items) > 1) {
            // delete order item price from order total price
    ...
    public function tracking_order($order_id)
    {
        ...
        // return view('users.suppliers.orders.track_order', compact('order'));
    }
