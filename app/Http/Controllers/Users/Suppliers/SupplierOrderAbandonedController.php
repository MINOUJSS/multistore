...
class SupplierOrderAbandonedController extends Controller
{
    ...
    public function unlock_phone_number($order_id)
    {
        ...
        if ($result >= 10) {
            ...
            $balance->outstanding_amount = $balance->outstanding_amount + get_order_abandoned_comition(get_supplier_data(auth()->user()->tenant_id)->plan_subscription->plan_id);
            $balance->update();
    ...
    {
        $abandoned_order = SupplierOrderAbandoned::findOrFail($request->order_id);
        // ctreate new order
        // return response()->json([
        //     'message' => 'success',
        ...
        try {
            $supplierOrder = SupplierOrders::create([
                'supplier_id' => $abandoned_order->supplier_id,
