...
class SupplierController extends Controller
{
    public function index()
    {
        ...
        $topProducts = SupplierOrderItems::select(
            ...
            DB::raw('SUM(supplier_order_items.quantity) as total_sold'),
            DB::raw('COUNT(supplier_order_items.id) as orders_count')
        )
        ->join('supplier_products', 'supplier_products.id', '=', 'supplier_order_items.product_id')
        ->where('supplier_products.supplier_id', get_supplier_data(auth()->user()->tenant_id)->id)
        ->groupBy('supplier_products.id', 'supplier_products.name', 'supplier_products.image')
        ->orderByDesc('total_sold') // ترتيب حسب الكمية المباعة
        ->take(5) // مثلاً نجيب أفضل 10 منتجات
        ->get();
        // get top viewed products
        $topViewed = SupplierProductsVisits::select(
            'supplier_products.id',
            'supplier_products.name',
            'supplier_products.image',
            DB::raw('COUNT(supplier_products_visits.id) as views_count')
        )
        ->join('supplier_products', 'supplier_products.id', '=', 'supplier_products_visits.product_id')
        ->where('supplier_products.supplier_id', get_supplier_data(auth()->user()->tenant_id)->id)
        ->groupBy('supplier_products.id', 'supplier_products.name', 'supplier_products.image')
        ->orderByDesc('views_count')
        ->take(10)
        ->get();
        //  $topViewed = SupplierProducts::where('supplier_id', get_supplier_data(auth()->user()->tenant_id)->id)
        ...
        // جلب الطلبات من DB
        $dailyOrdersRaw = DB::table('supplier_orders')
            ->select(
                DB::raw('DATE(created_at) as date'),
                DB::raw('COUNT(*) as total')
            )
            ->where('supplier_id', get_supplier_data(auth()->user()->tenant_id)->id)
        ...
        //     $dailyOrders = DB::table('supplier_orders')
        ...
        // جلب البيانات من DB
        $weeklyOrdersRaw = DB::table('supplier_orders')
            ->select(
        ...
        // // بيانات أسبوعية
        // $weeklyOrders = DB::table('supplier_orders')
        ...
        $monthlyOrdersRaw = DB::table('supplier_orders')
            ->select(
        ...
        // // بيانات شهرية
        // $monthlyOrders = DB::table('supplier_orders')
        ...
        // توزيع حالات الطلبات
        $statusCounts = DB::table('supplier_orders')
        ...
        // الأسبوع الحالي
        $currentWeekAllOrders = DB::table('supplier_orders')
            ->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])
            ->count();

        // الأسبوع الماضي
        $lastWeekAllOrders = DB::table('supplier_orders')
        ...
        // ---------------------------------------------
        // الشهر الحالي
        $currentMonthAllOrders = DB::table('supplier_orders')
            ->whereMonth('created_at', Carbon::now()->month)
            ->whereYear('created_at', Carbon::now()->year)
            ->count();

        // الشهر الماضي
        $lastMonthAllOrders = DB::table('supplier_orders')
        ...
        // الأسبوع الحالي
        $currentWeekConfirmedOrders = DB::table('supplier_orders')
            ->where('status', 'processing')
            ->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])
            ->count();

        // الأسبوع الماضي
        $lastWeekConfirmedOrders = DB::table('supplier_orders')
            ->where('status', 'processing')
            ->whereBetween('created_at', [now()->subWeek()->startOfWeek(), now()->subWeek()->endOfWeek()])
            ->count();
        ...
        // ---------------------------------------------
        // الشهر الحالي
        $currentMonthConfirmedOrders = DB::table('supplier_orders')
            ->where('status', 'processing')
            ->whereMonth('created_at', Carbon::now()->month)
            ->whereYear('created_at', Carbon::now()->year)
            ->count();
        ...
        // الشهر الماضي
        $lastMonthConfirmedOrders = DB::table('supplier_orders')
            ->where('status', 'processing')
            ->whereMonth('created_at', Carbon::now()->subMonth()->month)
            ->whereYear('created_at', Carbon::now()->subMonth()->year)
            ->count();
        ...
        // الأسبوع الحالي
        $currentWeekDeliveredOrders = DB::table('supplier_orders')
            ->where('status', 'delivered')
            ->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])
            ->count();

        // الأسبوع الماضي
        $lastWeekDeliveredOrders = DB::table('supplier_orders')
            ->where('status', 'delivered')
            ->whereBetween('created_at', [now()->subWeek()->startOfWeek(), now()->subWeek()->endOfWeek()])
            ->count();
        ...
        // ---------------------------------------------
        // الشهر الحالي
        $currentMonthDeliveredOrders = DB::table('supplier_orders')
            ->where('status', 'delivered')
            ->whereMonth('created_at', Carbon::now()->month)
            ->whereYear('created_at', Carbon::now()->year)
            ->count();
        ...
        // الشهر الماضي
        $lastMonthDeliveredOrders = DB::table('supplier_orders')
            ->where('status', 'delivered')
            ->whereMonth('created_at', Carbon::now()->subMonth()->month)
            ->whereYear('created_at', Carbon::now()->subMonth()->year)
            ->count();
        ...
        // الأسبوع الحالي
        $currentWeekCanceledOrders = DB::table('supplier_orders')
            ->where('status', 'canceled')
            ->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])
            ->count();

        // الأسبوع الماضي
        $lastWeekCanceledOrders = DB::table('supplier_orders')
            ->where('status', 'canceled')
            ->whereBetween('created_at', [now()->subWeek()->startOfWeek(), now()->subWeek()->endOfWeek()])
            ->count();
        ...
        // ---------- حساب نسبة الطلبات الملغاة -----------
        // الشهر الحالي
        $currentMonthCanceledOrders = DB::table('supplier_orders')
            ->where('status', 'canceled')
            ->whereMonth('created_at', Carbon::now()->month)
            ->whereYear('created_at', Carbon::now()->year)
            ->count();
        ...
        // الشهر الماضي
        $lastMonthCanceledOrders = DB::table('supplier_orders')
            ->where('status', 'canceled')
            ->whereMonth('created_at', Carbon::now()->subMonth()->month)
            ->whereYear('created_at', Carbon::now()->subMonth()->year)
            ->count();
        ...
        // الأسبوع الحالي
        $currentWeekAbandonedOrders = DB::table('supplier_order_abandoneds')
            ->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])
            ->count();

        // الأسبوع الماضي
        $lastWeekAbandonedOrders = DB::table('supplier_order_abandoneds')
            ->whereBetween('created_at', [now()->subWeek()->startOfWeek(), now()->subWeek()->endOfWeek()])
            ->count();
        ...
        // ---------- حساب نسبة الطلبات المتروكة -----------
        // الشهر الحالي
        $currentMonthAbandonedOrders = DB::table('supplier_order_abandoneds')
            ->whereMonth('created_at', Carbon::now()->month)
            ->whereYear('created_at', Carbon::now()->year)
            ->count();

        // الشهر الماضي
        $lastMonthAbandonedOrders = DB::table('supplier_order_abandoneds')
            ->whereMonth('created_at', Carbon::now()->subMonth()->month)
            ->whereYear('created_at', Carbon::now()->subMonth()->year)
            ->count();
    ...
    //     return view('users.suppliers.profile.index', compact('supplier', 'user', 'wilayas'));
    // }
