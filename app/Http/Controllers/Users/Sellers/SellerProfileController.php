...
class SellerProfileController extends Controller
{
    public function index()
    {
        ...
        return view('users.sellers.profile.index', compact('seller', 'user', 'wilayas', 'chargily_settings', 'bank_settings'));
    }
    ...
    public function update(Request $request)
    {
        ...
        $seller->birth_date = $request->birth_date;
        $seller->store_name = $request->store_name;
        if ($request->part_of_approved_list) {
            $seller->part_of_approved_list = 'yes';
        } else {
    ...
    public function request_validation(Request $request)
    {
        ...
        if ($exists) {
            ...
            // ], 422);
            return redirect()->route('seller.profile')->with('error', 'لديك طلب توثيق قيد المراجعة بالفعل.');
        }
        ...
        try {
            ...
            // ]);
            return redirect()->route('seller.profile')->with('success', 'تم إرسال طلب التوثيق بنجاح.');
        } catch (\Exception $e) {
            DB::rollBack();
