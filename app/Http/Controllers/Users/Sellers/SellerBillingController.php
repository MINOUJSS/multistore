...
class SellerBillingController extends Controller
{
    public function index()
    {
        ...
        return view('users.sellers.billing.index', compact('invoices'));
    }
    ...
    public function view($id)
    {
        ...
        return view('users.sellers.billing.view', compact('data'));
    }
    ...
    public function invoice_redirect(Request $request, $invoiceId)
    {
        ...
        if ($request->payment_method == 'wallet') {
            return view('users.sellers.payments.wallet.invoice.index', compact('invoice'));
        }
        // if payment method is credit-card
        elseif ($request->payment_method == 'credit-card') {
            return view('users.sellers.payments.cib.invoice.index', compact('invoice'));
        }
        // if payment method is baridi-mob
        elseif ($request->payment_method == 'baridi-mob') {
            return view('users.sellers.payments.baridimob.invoice.index', compact('invoice'));
        }
        // if payment method is ccp
        else {
            return view('users.sellers.payments.ccp.invoice.index', compact('invoice'));
        }
    }
    ...
    public function pay_invoice(Request $request)
    {
        ...
        if ($request->payment_method == 'wallet') {
            if (auth()->user()->balance->balance < auth()->user()->balance->outstanding_amount) {
                ...
                return redirect()->route('seller.billing')->with('paid', 'تم الدفع المبلغ من رصيدك في المنصة بنجاح');
            }
        } else {
            ...
            if (Storage::disk('public')->exists('seller/'.get_seller_store_name(auth()->user()->tenant_id).'/payment_proofs/'.$invoice->invoice_number)) {
                return redirect()->route('seller.billing')->with('error', 'يوجد ملف إثبات دفع بنفس الاسم تم رفعه سابقاً.');
            }
            // حفظ الملف داخل مجلد secure
            $proofPath = $request->file('payment_proof')->store('seller/'.get_seller_store_name(auth()->user()->tenant_id).'/payment_proofs/'.$invoice->invoice_number, 'public');
            ...
            return redirect()->route('seller.billing')->with('paid', 'تم إرسال إيصال الدفع بنجاح، وسيتم مراجعته قريباً.');
        }
    ...
    {
        $folderPath = 'seller/'.get_seller_store_name(auth()->user()->tenant_id).'/payment_proofs/'.$invoice->invoice_number;
        if ($invoice->status != 'paid') {
            if ($invoice->payment_proof && Storage::disk('public')->exists($invoice->payment_proof)) {
                ...
                return redirect()->route('seller.billing')->with('success', 'تم حذف مجلد إيصال الدفع بنجاح.');
            }
        }

        return redirect()->route('seller.billing')->with('error', 'لم يتم العثور على ملف إثبات الدفع.');
    }
}
