...
class SellerProofsRefusedChatController extends Controller
{
    ...
    public function index($proofId)
    {
        ...
        return view('users.sellers.components.content.proofs_refused.chat_box', compact('proof'));
    }
    ...
    public function fetchMessages($proofId)
    {
        ...
        $unreadCount = $messages->where('sender_type', 'admin')->where('is_read_by_seller', false)->count(); // Admin messages are unread for seller
    ...
    public function sendMessage(Request $request, $proofId)
    {
        ...
        if ($request->hasFile('attachments')) {
            foreach ($request->file('attachments') as $file) {
                ...
                $path = Storage::disk('general')->put('proofs_refused_chats/'.$proofId.'/seller', $file);
                $attachments[] = str_replace('public/', '', $path);
        ...
        $newMessage = ProofsRefusedMessage::create([
            ...
            'message' => $messageText,
            'sender_type' => 'user', // This is the seller sending the message
