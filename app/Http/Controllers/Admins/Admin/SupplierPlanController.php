...
{
    /**
     * Display a listing of the supplier plans.
     */
    public function index()
    {
        $plans = SupplierPlan::withCount(['pricing', 'Authorizations', 'subscriptions'])
            ->orderBy('id', 'asc')
            ->get();
        ...
        return view('admins.admin.supplier_plans.index', compact(
            'plans',
            'totalPlans',
            'totalSubscriptions',
            'freePlansCount',
    ...
    /**
     * Show the form for creating a new supplier plan.
     */
    public function create()
    {
        return view('admins.admin.supplier_plans.create');
    }

    /**
     * Store a newly created supplier plan in storage.
     */
    public function store(Request $request)
    {
        $request->validate([
            'name' => 'required|string|max:191|unique:supplier_plans,name',
            'description' => 'nullable|string',
            'price' => 'required|numeric|min:0',
        ], [
            'name.required' => 'يرجى إدخال اسم الخطة.',
            'name.unique' => 'اسم هذه الخطة مسجل مسبقاً.',
            'price.required' => 'يرجى إدخال السعر الافتراضي للخطة.',
            'price.numeric' => 'يجب أن يكون السعر قيمة رقمية.',
        ]);

        $plan = SupplierPlan::create([
            'name' => $request->name,
            'description' => $request->description,
            'price' => $request->price,
        ]);

        return redirect()->route('admin.supplier_plans.show', $plan->id)
            ->with('success', 'تم إنشاء الخطة بنجاح! يمكنك الآن إدارة فترات التسعير والصلاحيات الخاصة بها.');
    }

    /**
     * Display the specified supplier plan with all pricing and authorizations.
     */
    public function show($id)
    {
        $plan = SupplierPlan::with([
            'pricing' => function ($q) {
        ...
        // Predefined common permissions presets for suppliers
        $presets = [
            ...
            ['key' => 'max_products', 'label' => 'أقصى عدد منتجات مسموح به (أدخل رقماً أو 0 لغير محدود)', 'default_value' => '50'],
        ...
        return view('admins.admin.supplier_plans.show', compact('plan', 'presets'));
    }

    /**
     * Show the form for editing the specified supplier plan.
     */
    public function edit($id)
    {
        $plan = SupplierPlan::findOrFail($id);
        return view('admins.admin.supplier_plans.edit', compact('plan'));
    }

    /**
     * Update the specified supplier plan in storage.
     */
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            'name' => 'required|string|max:191|unique:supplier_plans,name,' . $id,
            'description' => 'nullable|string',
            'price' => 'required|numeric|min:0',
        ], [
            'name.required' => 'يرجى إدخال اسم الخطة.',
    ...
    /**
     * Remove the specified supplier plan from storage.
     */
    public function destroy($id)
    {
        ...
        if ($plan->subscriptions()->count() > 0) {
            return redirect()->back()->with('error', 'لا يمكن حذف هذه الخطة لوجود ' . $plan->subscriptions()->count() . ' اشتراك(ات) موردين مرتبطين بها حالياً.');
        }

        $plan->delete();

        return redirect()->route('admin.supplier_plans.index')
    ...
    /**
     * Store a pricing tier for the supplier plan.
     */
    public function storePrice(Request $request, $planId)
    {
        $plan = SupplierPlan::findOrFail($planId);
        ...
        $exists = SupplierPlanPrices::where('plan_id', $planId)
    ...
    {
        $priceTier = SupplierPlanPrices::findOrFail($priceId);
        ...
        $duplicate = SupplierPlanPrices::where('plan_id', $priceTier->plan_id)
            ->where('duration', $request->duration)
            ->where('id', '!=', $priceId)
            ->exists();
    ...
    /**
     * Store a new authorization/permission for the supplier plan.
     */
    public function storeAuthorization(Request $request, $planId)
    {
        ...
        $request->validate([
            ...
            'description.required' => 'يرجى إدخال وصف الصلاحية الظاهر للمورد.',
        ]);

        $exists = SupplierPlanAuthorizations::where('plan_id', $planId)
        ...
        SupplierPlanAuthorizations::create([
            'plan_id' => $planId,
    ...
    {
        $authorization = SupplierPlanAuthorizations::findOrFail($authId);
        ...
        $duplicate = SupplierPlanAuthorizations::where('plan_id', $authorization->plan_id)
            ->where('permission_key', $request->permission_key)
            ->where('id', '!=', $authId)
            ->exists();
    ...
    /**
     * Delete an authorization from the supplier plan.
     */
