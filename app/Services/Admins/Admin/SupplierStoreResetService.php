...
class SupplierStoreResetService
{
    /**
     * Reset the supplier's storefront presentation elements to initial defaults
     ...
     * @throws \Throwable
     */
    public function reset(Supplier $supplier, User $user): void
    {
        DB::transaction(function () use ($supplier, $user) {
            ...
            // 3. Reset Store FAQs (SupplierFqa)
            SupplierFqa::where('supplier_id', $supplierId)->delete();
            $defaultFqas = [
                [
            ...
            foreach ($defaultFqas as $fqa) {
                SupplierFqa::create($fqa);
            }

            // 4. Reset Static Pages (SupplierPage)
            SupplierPage::where('supplier_id', $supplierId)->delete();
            $defaultPages = [
                [
                    'title' => 'عن المتجر',
            ...
            Log::info("Supplier store successfully reset to defaults for supplier ID: {$supplierId}, User ID: {$userId}");
        });
    }
}
