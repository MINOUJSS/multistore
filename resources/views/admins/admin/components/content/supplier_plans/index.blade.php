        ...
        <div class="row align-items-center position-relative z-1">
            <div class="col-lg-8 mb-3 mb-lg-0">
                ...
                <h1 class="display-6 fw-bold mb-2 text-white text-start">
                    📦 خطط اشتراك الموردين (Supplier Plans)
                </h1>
                <p class="text-white-50 mb-0 leading-relaxed text-start">
                    التحكم الشامل في خطط الموردين، مدد التسعير المتعددة (30، 90، 180، 365 يوماً)، وإدارة مميزات وصلاحيات كل خطة بدقة.
                </p>
            </div>
            <div class="col-lg-4 text-lg-end">
                <div class="d-flex flex-wrap gap-2 justify-content-lg-end">
                    <a href="{{ route('admin.supplier_plans.create') }}" class="btn btn-warning text-dark fw-bold px-3 py-2 rounded-3 border-0 shadow-sm">
        ...
        <div class="col-xl-3 col-sm-6">
            <div class="card border-0 shadow-sm rounded-4 h-100 bg-white dashboard-stat-card">
                <div class="card-body p-3">
                    ...
                    <h6 class="text-muted fw-semibold small mb-1">إجمالي الخطط</h6>
        ...
        <div class="col-xl-3 col-sm-6">
            <div class="card border-0 shadow-sm rounded-4 h-100 bg-white dashboard-stat-card">
                <div class="card-body p-3">
                    ...
                    <h6 class="text-muted fw-semibold small mb-1">إجمالي الاشتراكات</h6>
    ...
    <div class="card border-0 shadow-sm rounded-4 w-100 overflow-hidden" style="max-width: 100%;">
        <div class="card-header bg-white border-0 fw-bold py-3 px-4 d-flex align-items-center justify-content-between flex-wrap gap-2">
            <div class="d-flex align-items-center gap-2">
                ...
                <span class="fs-6">قائمة خطط الموردين المتوفرة</span>
            </div>
            <div class="d-flex align-items-center gap-2">
                <span class="badge bg-light text-dark border px-3 py-1.5 rounded-pill">
                    {{ $plans->count() }} خطة
                </span>
                <a href="{{ route('admin.supplier_plans.create') }}" class="btn btn-sm btn-primary rounded-3 px-3 shadow-sm" style="background-color: #a40c72; border-color: #a40c72;">
                    <i class="fa-solid fa-plus me-1"></i> إضافة خطة
                </a>
            </div>
        </div>
                ...
                <tbody class="text-center">
                    @forelse ($plans as $plan)
                        <tr>
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div>
                                        <h6 class="mb-0 fw-bold text-dark">
                                            <a href="{{ route('admin.supplier_plans.show', $plan->id) }}" class="text-dark text-decoration-none hover-primary">
                                                {{ $plan->name }}
                                            </a>
                                        </h6>
                            ...
                            <td>
                                <a href="{{ route('admin.supplier_plans.show', $plan->id) }}" class="text-decoration-none">
                            ...
                            <td>
                                <span class="badge bg-secondary-subtle text-secondary px-3 py-1.5 rounded-pill fw-bold">
                                    <i class="fa-solid fa-user-check me-1"></i> {{ $plan->subscriptions_count }} مورد
                                </span>
                            </td>
                            <td>
                                <div class="d-flex justify-content-center align-items-center gap-1 flex-wrap">
                                    <a href="{{ route('admin.supplier_plans.show', $plan->id) }}"
                                    ...
                                    <a href="{{ route('admin.supplier_plans.edit', $plan->id) }}"
                                       class="btn btn-sm btn-outline-secondary rounded-3 px-2 py-1"
                                       title="تعديل الخطة">
                                        <i class="fa fa-edit"></i>
                                    </a>

                                    <form action="{{ route('admin.supplier_plans.destroy', $plan->id) }}"
                                          method="POST"
                                          onsubmit="return confirm('هل أنت متأكد من حذف الخطة ({{ $plan->name }})؟ سيتم حذف جميع فترات التسعير والصلاحيات المرتبطة بها.')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-muted py-5 text-center">
                                <div class="py-4">
                                    <i class="fa-solid fa-cubes-stacked fs-1 mb-3 d-block text-muted opacity-50"></i>
                                    <h5 class="fw-bold">لا توجد أي خطط للموردين مسجلة حتى الآن</h5>
                                    <p class="text-muted small">يمكنك البدء بإضافة أول خطة موردين للمنصة عبر الزر أدناه</p>
                                    <a href="{{ route('admin.supplier_plans.create') }}" class="btn btn-primary rounded-3 px-4 py-2 mt-2 shadow-sm" style="background-color: #a40c72; border-color: #a40c72;">
