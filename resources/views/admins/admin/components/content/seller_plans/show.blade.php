        ...
        <div class="row align-items-center position-relative z-1">
            <div class="col-lg-8 mb-3 mb-lg-0">
                ...
                <h1 class="display-6 fw-bold mb-2 text-white text-start">
                    ⚙️ إعدادات خطة البائع: {{ $plan->name }}
                </h1>
                <p class="text-white-50 mb-0 leading-relaxed text-start">
                    إدارة فترات التسعير المتعددة (30، 90، 180، 365 يوماً)، وتعيين صلاحيات ومميزات المتجر لتجار التجزئة المنضمين لهذه الخطة.
                </p>
            </div>
            <div class="col-lg-4 text-lg-end">
                <div class="d-flex flex-wrap gap-2 justify-content-lg-end">
                    <a href="{{ route('admin.seller_plans.edit', $plan->id) }}" class="btn btn-warning text-dark fw-bold px-3 py-2 rounded-3 border-0 shadow-sm">
                        <i class="fa-solid fa-pen-to-square me-1"></i> تعديل بيانات الخطة
                    </a>
                    <a href="{{ route('admin.seller_plans.index') }}" class="btn btn-outline-light text-white fw-bold px-3 py-2 rounded-3 border-2 shadow-sm">
                ...
                <div class="col-md-3 border-start-md">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            ...
                            <h5 class="fw-bold mb-0 text-dark">{{ $plan->subscriptions->count() }} <small class="fs-7 text-muted">تاجر</small></h5>
                        </div>
                    </div>
                </div>
                            ...
                            <td>
                                <div class="d-flex justify-content-center align-items-center gap-1">
                                    <button type="button"
                                            ...
                                            data-action="{{ route('admin.seller_plans.prices.update', $priceTier->id) }}"
                                            title="تعديل المدة أو السعر">
                                        <i class="fa fa-edit"></i>
                                    </button>

                                    <form action="{{ route('admin.seller_plans.prices.destroy', $priceTier->id) }}"
                                          method="POST"
        ...
        <div class="table-responsive p-0">
            <table class="table table-hover align-middle mb-0">
                <thead class="bg-light text-muted small text-center">
                    <tr>
                        <th class="py-3">#</th>
                        <th class="py-3 text-start ps-4">الميزة / الوصف المعروض للبائع</th>
                ...
                <tbody class="text-center">
                    @forelse($plan->Authorizations as $auth)
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('admin.seller_plans.authorizations.toggle', $auth->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('PATCH')
                            ...
                            <td>
                                <div class="d-flex justify-content-center align-items-center gap-1">
                                    <button type="button"
                                            ...
                                            data-action="{{ route('admin.seller_plans.authorizations.update', $auth->id) }}"
                                            title="تعديل الصلاحية">
                                        <i class="fa fa-edit"></i>
                                    </button>

                                    <form action="{{ route('admin.seller_plans.authorizations.destroy', $auth->id) }}"
                                          method="POST"
...
<div class="modal fade" id="addPriceModal" tabindex="-1" aria-labelledby="addPriceModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            ...
            <form action="{{ route('admin.seller_plans.prices.store', $plan->id) }}" method="POST">
                @csrf
...
<div class="modal fade" id="addAuthModal" tabindex="-1" aria-labelledby="addAuthModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            ...
            <form action="{{ route('admin.seller_plans.authorizations.store', $plan->id) }}" method="POST">
                @csrf
                <div class="modal-body p-4">
                    ...
                    <div class="mb-4 p-3 rounded-3" style="background-color: rgba(164,12,114,0.06); border: 1px dashed rgba(164,12,114,0.3);">
                        <label class="form-label fw-bold text-dark d-flex align-items-center gap-2 mb-1">
                            ...
                            <span>اختيار مسبق من ميزات النظام الشائعة لتجار التجزئة (Presets)</span>
                        </label>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            <label for="description" class="form-label fw-semibold text-dark">
                                وصف الميزة الظاهر للبائع <span class="text-danger">*</span>
                            </label>
                            <input type="text"
                                   name="description"
                                   id="description"
                                   class="form-control rounded-3"
                                   placeholder="مثال: المنتجات المسموح بها (100 منتج)"
                                   required>
                            <small class="text-muted">النص الذي يراه البائع في جدول مقارنة الخطط وفي لوحة تحكمه.</small>
                        </div>
