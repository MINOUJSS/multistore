...
<div id="chatBoxContainer" class="card shadow-lg border-0"
    ...
    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <div>
            🧾 <strong>دردشة النزاع رقم #{{ $dispute->id ?? '—' }}</strong><br>
            <small>موضوع: {{ $dispute->subject ?? '—' }}</small>
        </div>
    ...
    <!--صوت التنبيه -->
    <audio id="messageSound" src="{{ asset('asset/v1/users/dashboard/sounds/notification.mp3') }}"
        preload="auto"></audio>
</div>
