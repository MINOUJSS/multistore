...
<div class="checkout-container">
    <h2 class="mb-4 text-center">
        ...
        <img src="{{asset('asset/v1/users/store')}}/img/payments/bank.png" class="img-fluid" alt="chargily" width="200px"/>
    </h2>
        ...
        <div class="col-lg-7 col-md-6">
            <div class="sticky-top" style="top: 20px; z-index: 1019;">
                ...
                <form>
                  ...
                  <input type="name" name="name" class="form-control @error('name') is-invalid @enderror" id="name" value="@if(old('name')){{ old('name') }}@else{{ $order->customer_name }}@endif" disabled>
                  @error('name')
                ...
                <div class="col-md-6">
                  ...
                  <input type="phone"  name="phone" class="form-control @error('phone') is-invalid @enderror" id="phone" value="@if(old('phone')){{ old('phone') }}@else{{ $order->phone }}@endif" disabled>
                  @error('phone')
                  <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
                </div>
                <div class="col-12">
                  ...
                  <input type="text" name="address" class="form-control @error('address') is-invalid @enderror" id="address" value="@if(old('address')){{ old('address') }}@else{{ $order->shipping_address  }}@endif" placeholder="الحي، الشارع، رقم المنزل" disabled>
                  @error('address')
                  <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
                </div>
                <div class="col-md-4">
                    ...
                    <select id="inputWilaya" name="wilaya" class="form-select @error('wilaya') is-invalid @enderror" disabled>
                      <option value="0" selected>إختر الولاية...</option>
                      @foreach ($wilayas as $wilaya)
                          <option value="{{ $wilaya->id }}" {{ old('wilaya') == $wilaya->id || $order->wilaya_id == $wilaya->id ? 'selected' : '' }}>
                ...
                <div class="col-md-4">
                  ...
                  <select id="inputDayra" name="dayra" class="form-select @error('dayrea') is-invalid @enderror" disabled>
                    <option value="0" selected>غير محددة ...</option>
                    @if($order->wilaya_id != null)
                    @foreach (get_wilaya_data($order->wilaya_id)->dayras as $dayra)
                    <option value="{{ $dayra->id }}" {{ old('dayra') == $dayra->id || $order->dayra_id == $dayra->id ? 'selected' : '' }}>
                ...
                <div class="col-md-4">
                    ...
                    <select id="inputBaladia" name="baladia" class="form-select @error('baladia') is-invalid @enderror" disabled>
                        <option value="0" selected>غير محددة ...</option>
                        ...
                        <option value="{{ $baladia->id }}" {{ old('baladia') == $baladia->id || $order->baladia_id == $baladia->id ? 'selected' : '' }}>
            ...
            <div class="sticky-top" style="top: 20px; z-index: 1019;">
                ...
                <div class="order-summary">
                    ...
                    <div class="product-item d-flex align-items-start">
                        <img src="{{$item->product->image}}" alt="{{$item->product->name}}" class="me-3 rounded" width="60" height="60">
                        <div class="flex-grow-1">
                            ...
                            <small class="text-muted d-block">{{$item->attribute->attribute->name}} : {{$item->attribute->value}}</small>
                            @endif
                        </div>
                    </div>  
                    @php
                    ...
                    {
                        $discount+=($item->product->price - $item->product->activeDiscount->discount_amount) * $item->quantity;
                    }                    
                    $sub_total+= $item->product->price * $item->quantity;
                    (supplier_product_has_discount($item->product->id)) ? $total+= $item->product->activeDiscount->discount_amount * $item->quantity : $total+= $item->product->price * $item->quantity;   
                    @endphp
                    ...
                    <form action="{{route('tenant.payments.verments_pay')}}" method="POST" enctype="multipart/form-data">
                        ...
                        <input type="file" name="payment_proof" id="payment_proof" class="form-control mb-3 mt-3 @error ('payment_proof') is-invalid @enderror" accept="application/pdf, image/jpeg, image/png" required>
                        @error('payment_proof')
