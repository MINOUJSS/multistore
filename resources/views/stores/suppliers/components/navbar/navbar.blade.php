    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
        <div class="navbar-nav hstacks gap-3 mx-auto">
          <a class="nav-link active" aria-current="page" href="{{url(request()->server('REQUEST_SCHEME').'://'.get_tenant_data(tenant('id'))->domains[0]->domain)}}">الرئيسية</a>
          <a class="nav-link" href="{{url(request()->server('REQUEST_SCHEME').'://'.get_tenant_data(tenant('id'))->domains[0]->domain)}}/contact-us">إتصل بنا</a>
          <a class="nav-link" href="{{url(request()->server('REQUEST_SCHEME').'://'.get_tenant_data(tenant('id'))->domains[0]->domain)}}/categories">التصنيفات</a>
