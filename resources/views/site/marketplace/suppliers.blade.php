...
@section('hero')
<!-- ======= Suppliers Marketplace Hero Header ======= -->
<section id="hero" class="d-flex align-items-center position-relative overflow-hidden py-5" style="min-height: 38vh;">
    ...
    <div class="container position-relative z-index-2 py-4 text-center">
        ...
        <div class="d-inline-flex align-items-center gap-2 mb-3 hero-badge-pill">
            ...
            <span class="badge-text fw-bold">{{ __('site.suppliers_marketplace') }}</span>
        </div>

        <!-- Main Title -->
        <h1 class="hero-main-title fw-bold text-white mb-3 fs-1">
            {{ __('site.marketplace_suppliers_title') }}
        </h1>

        <!-- Subtitle -->
        <p class="hero-sub-title mb-4 fs-6 mx-auto text-white-80" style="max-width: 740px;">
            {{ __('site.marketplace_suppliers_subtitle') }}
        </p>
    ...
    <style>
        #hero {
            background: linear-gradient(135deg, #100619 0%, #290e38 45%, #4b1760 85%, #8B2577 100%);
        }
        .hero-glow-1 {
            position: absolute;
            width: 420px;
            height: 420px;
            background: radial-gradient(circle, rgba(139, 37, 119, 0.45) 0%, rgba(139, 37, 119, 0) 70%);
            top: -10%;
            left: -5%;
            filter: blur(50px);
            pointer-events: none;
        ...
        .hero-glow-2 {
            ...
            width: 380px;
            height: 380px;
            background: radial-gradient(circle, rgba(216, 112, 196, 0.3) 0%, rgba(216, 112, 196, 0) 70%);
            bottom: -10%;
            right: -5%;
            filter: blur(40px);
            pointer-events: none;
...
<section class="marketplace-section py-5" style="background: #f8f9fc; min-height: 60vh;">
    <div class="container" data-aos="fade-up">
        ...
        <div class="card border-0 shadow-sm rounded-4 mb-4 filter-card p-4">
            <form action="{{ route('site.marketplace.suppliers') }}" method="GET" class="row g-3 align-items-end">
                
                <!-- Search Input -->
                <div class="col-lg-4 col-md-6">
                    <label class="form-label fw-bold text-dark small mb-1">
                        <i class="bi bi-search text-purple me-1"></i> {{ __('site.filter_search_placeholder') }}
                    </label>
                ...
                <div class="col-lg-3 col-md-6">
                    <label class="form-label fw-bold text-dark small mb-1">
                        <i class="bi bi-grid text-purple me-1"></i> {{ __('site.filter_category') }}
                    </label>
                ...
                <div class="col-lg-2 col-md-4 col-6">
                    <label class="form-label fw-bold text-dark small mb-1">
                        <i class="bi bi-boxes text-purple me-1"></i> {{ __('site.filter_moq') }}
                    </label>
                    <input type="number" name="max_moq" class="form-control rounded-3" 
                           placeholder="مثال: 10" value="{{ request('max_moq') }}" min="1">
                </div>

                <!-- Sort Filter -->
                <div class="col-lg-3 col-md-4 col-6">
                    <label class="form-label fw-bold text-dark small mb-1">
                        <i class="bi bi-sort-down text-purple me-1"></i> {{ __('site.filter_sort') }}
                    </label>
                    <select name="sort" class="form-select rounded-3">
                        ...
                        <option value="moq_asc" {{ request('sort') == 'moq_asc' ? 'selected' : '' }}>{{ __('site.sort_moq_low_high') }}</option>
                ...
                <div class="col-lg-3 col-md-4 col-6">
                    <label class="form-label fw-bold text-dark small mb-1">{{ __('site.filter_min_price') }} (د.ج)</label>
                    <input type="number" name="min_price" class="form-control rounded-3" 
                           placeholder="0" value="{{ request('min_price') }}" min="0" step="100">
                </div>

                <div class="col-lg-3 col-md-4 col-6">
                    <label class="form-label fw-bold text-dark small mb-1">{{ __('site.filter_max_price') }} (د.ج)</label>
                    <input type="number" name="max_price" class="form-control rounded-3" 
                           placeholder="100000" value="{{ request('max_price') }}" min="0" step="100">
                </div>

                <div class="col-lg-6 col-md-4 d-flex gap-2">
                    <button type="submit" class="btn btn-primary-purple flex-grow-1 rounded-3 py-2 fw-semibold d-inline-flex align-items-center justify-content-center gap-2">
                        <i class="bi bi-funnel"></i>
                        <span>{{ __('site.apply_filters') }}</span>
                    </button>
                    <a href="{{ route('site.marketplace.suppliers') }}" class="btn btn-outline-secondary rounded-3 py-2 px-3 fw-semibold" title="{{ __('site.reset_filters') }}">
        ...
        <div class="d-flex flex-wrap align-items-center justify-content-between mb-4 px-2">
            <div class="d-flex align-items-center gap-2">
                <span class="badge bg-purple-light text-purple fs-6 px-3 py-2 rounded-pill fw-bold">
                    <i class="bi bi-box-seam me-1"></i> {{ $products->total() }} منتج جملة
                </span>
                <span class="text-muted small">{{ __('site.total_products_available') }} {{ $totalProductsCount }}</span>
            </div>
            @if(request()->hasAny(['search', 'category_id', 'min_price', 'max_price', 'max_moq', 'sort']))
            ...
            <div class="row g-4">
                @foreach($products as $product)
                    @php
                        ...
                        $tenantDomain = optional(optional(optional($product->supplier)->tenant)->domains)->first();
                        ...
                        // Product Image
                        $productImage = $product->image ?: asset('asset/v1/site/defaulte/img/portfolio/portfolio-2.jpg');
                            ...
                            <div class="product-img-wrapper position-relative overflow-hidden bg-light text-center">
                                <a href="{{ $productUrl }}" target="_blank" rel="noopener noreferrer">
                                    <img src="{{ $productImage }}" alt="{{ $product->name }}" 
                                         ...
                                         onerror="this.src='{{ asset('asset/v1/site/defaulte/img/portfolio/portfolio-2.jpg') }}'"
                            ...
                            <div class="card-body p-3.5 d-flex flex-column justify-content-between">
                                <div>
                                    ...
                                    <div class="d-flex align-items-center justify-content-between mb-2">
                                        <a href="{{ $storeUrl }}" target="_blank" rel="noopener noreferrer" 
                                           class="supplier-badge text-decoration-none d-inline-flex align-items-center gap-1.5 small fw-semibold text-truncate"
                                           style="max-width: 80%;">
                                            <i class="bi bi-patch-check-fill text-purple"></i>
                                            <span class="text-truncate">{{ optional($product->supplier)->store_name ?? 'مورد جملة معتمد' }}</span>
                                        </a>
                                        <span class="badge bg-primary-subtle text-primary rounded-pill small px-2 py-0.5">
                                            جملة
                                        </span>
                                    </div>
                                    ...
                                    <h5 class="card-title fs-6 fw-bold text-dark mb-2 text-truncate-2" title="{{ $product->name }}">
                                        <a href="{{ $productUrl }}" target="_blank" rel="noopener noreferrer" class="text-dark text-decoration-none hover-purple">
                                            {{ $product->name }}
                                        </a>
                                    </h5>

                                    <!-- Wholesale Specs (MOQ & Stock) -->
                                    <div class="wholesale-specs bg-light rounded-3 p-2.5 mb-2.5 d-flex justify-content-between align-items-center text-muted small">
                                        <div>
                                            <i class="bi bi-box me-1 text-purple"></i>
                                            <span>{{ __('site.min_order_qty') }}:</span>
                                            <strong class="text-dark">{{ $product->minimum_order_qty ?? 1 }} {{ __('site.piece') }}</strong>
                                        </div>
                                        @if($product->qty)
                                            <div class="border-start ps-2">
                                                <i class="bi bi-archive me-1 text-success"></i>
                                                <span>{{ __('site.available_stock') }}:</span>
                                                <strong class="text-dark">{{ $product->qty }}</strong>
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    @if($product->short_description)
                                        <p class="card-text text-muted small text-truncate-2 mb-2">
                                            {{ Str::limit(strip_tags($product->short_description), 70) }}
                                        </p>
                                    @endif
                                </div>
                                ...
                                <div class="pt-2 border-top border-light-subtle mt-2">
                                    <div class="d-flex align-items-baseline justify-content-between mb-2">
                                        <div class="price-box">
                                            <span class="small text-muted d-block" style="font-size: 0.75rem;">سعر التوريد / الجملة</span>
                                            <span class="fw-bolder text-purple fs-5">{{ number_format($finalPrice, 2) }}</span>
                                    ...
                                    <a href="{{ $productUrl }}" target="_blank" rel="noopener noreferrer" 
                                       class="btn btn-primary-purple w-100 rounded-3 py-2 fw-semibold d-inline-flex align-items-center justify-content-center gap-2 shadow-sm">
                                        <span>{{ __('site.order_wholesale') }}</span>
        ...
        @else
            ...
            <div class="text-center py-5 my-4 bg-white rounded-4 shadow-sm p-5">
                <div class="empty-icon-box rounded-circle mx-auto mb-3 d-flex align-items-center justify-content-center"
                     style="width: 80px; height: 80px; background: rgba(123, 31, 162, 0.1); color: #7B1FA2;">
                ...
                <a href="{{ route('site.marketplace.suppliers') }}" class="btn btn-primary-purple rounded-pill px-4 py-2 fw-semibold">
...
<style>
    :root {
        ...
        --color-primary-purple-hover: #5c147a;
        --color-primary-purple-light: rgba(123, 31, 162, 0.1);
    }
    .text-purple {
    ...
    .product-market-card:hover {
        ...
        box-shadow: 0 16px 32px rgba(123, 31, 162, 0.12) !important;
