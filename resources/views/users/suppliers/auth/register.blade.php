...
@section('clarity')
@if(
          get_platform_data('clarity') &&
          get_platform_data('clarity')->status == 'active' &&
          !empty(get_platform_data('clarity')->value)
      )

      @php
          $measurementId = get_platform_data('clarity')->value;
      @endphp
        ...
        <div class="dz-auth-box">
          ...
          <form method="POST" action="{{ route('supplier.register') }}">
            ...
            <div class="mb-3">
                ...
                <div class="input-group mb-3">
                <input type="password" id="confirmPassword" name="password_confirmation" class="form-control @error('password_confirmation') is-invalid @enderror" placeholder="تأكيد كلمة المرور" required>
            ...
            <div class="mb-3 form-check">
                <label class="form-check-label" for="exampleCheck1">لديك حساب على منصتنا؟<a href="{{route('supplier.login')}}" class="forget-link">سجل الدخول</a></label>
              </div>
          </form>
        </div>
      </div>
...
@section('footer_js')
    @include('users.suppliers.auth.js.show_password_js')
    @include('users.suppliers.auth.js.show_confirm_password_js')
@endsection