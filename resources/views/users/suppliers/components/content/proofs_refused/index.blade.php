<div class="container-fluid py-3 px-3 px-md-4">
    ...
    <div class="proofs-hero p-4 p-md-5 mb-4 shadow-sm text-white position-relative overflow-hidden"
         style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 40%, #334155 100%); border-radius: 1.25rem;">
        <div class="row align-items-center position-relative z-1">
            <div class="col-lg-8 mb-3 mb-lg-0">
                <div class="d-inline-flex align-items-center gap-2 px-3 py-1 bg-white bg-opacity-10 rounded-pill text-white small mb-3 border border-white border-opacity-15 backdrop-blur">
                    ...
                    <span class="fw-semibold">{{ __('إثباتات الدفع المرفوضة للمورد') }}</span>
            ...
            <div class="col-lg-4 text-lg-end">
                <div class="d-flex flex-wrap gap-2 justify-content-lg-end">
                    ...
                    @if(Route::has('supplier.billing.invoice.create'))
                        <a href="{{ route('supplier.billing.invoice.create') }}" class="btn btn-outline-light text-white fw-bold px-3 py-2.5 rounded-3 border-2 shadow-sm d-inline-flex align-items-center gap-2">
        ...
        <div class="position-absolute rounded-circle bg-info opacity-10" style="width: 260px; height: 260px; top: -60px; left: -60px; pointer-events: none; filter: blur(50px);"></div>
        <div class="position-absolute rounded-circle bg-primary opacity-15" style="width: 200px; height: 200px; bottom: -40px; right: 10%; pointer-events: none; filter: blur(40px);"></div>
    </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 h-100 bg-white dashboard-stat-card">
                <div class="card-body p-3">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        ...
                        <span class="badge bg-indigo-subtle text-indigo px-2.5 py-1 rounded-pill fw-semibold small">الإجمالي</span>
                    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4 bg-white">
        <div class="card-header bg-white border-0 pt-4 px-4 pb-0 d-flex align-items-center justify-content-between">
            <div class="d-flex align-items-center gap-2">
                <div class="filter-icon-box bg-indigo-subtle text-indigo rounded-3 p-2">
            ...
            @if(request()->hasAny(['search', 'admin', 'date', 'user']))
                <a href="{{ Route::has('supplier.payments_proofs_refuseds') ? route('supplier.payments_proofs_refuseds') : url()->current() }}" class="btn btn-sm btn-light text-muted rounded-pill px-3 border-0">
                    <i class="fa-solid fa-xmark me-1"></i> إلغاء التصفية
                </a>
            @endif
        </div>
        <div class="card-body p-4">
            <form action="{{ Route::has('supplier.payments_proofs_refuseds') ? route('supplier.payments_proofs_refuseds') : url()->current() }}" method="GET">
                <div class="row g-3">
                    <!-- Live Search / General Search -->
                    <div class="col-lg-5 col-md-6">
                        <label for="liveSearchInput" class="form-label fw-semibold text-secondary small">
                            <i class="fa-solid fa-magnifying-glass me-1 text-indigo"></i> بحث سريع (السبب / رقم الإثبات / المستخدم)
                        </label>
                    ...
                    <div class="col-lg-3 col-md-6">
                        <label for="search_admin" class="form-label fw-semibold text-secondary small">
                            <i class="fa-solid fa-user-shield me-1 text-indigo"></i> المسؤول المراجع
                        </label>
                    ...
                    <div class="col-lg-2 col-md-6">
                        <label for="search_date" class="form-label fw-semibold text-secondary small">
                            <i class="fa-solid fa-calendar me-1 text-indigo"></i> تاريخ الإرسال
                        </label>
                    ...
                    <div class="col-lg-2 col-md-6 d-flex align-items-end">
                        <button type="submit" class="btn btn-indigo w-100 py-2 rounded-3 fw-semibold shadow-sm d-flex align-items-center justify-content-center gap-2">
    ...
    <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden mb-4">
        <div class="card-header bg-white border-0 py-3 px-4 d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div class="d-flex align-items-center gap-2">
                <span class="badge bg-indigo-subtle text-indigo p-2 rounded-circle">
                    <i class="fa-solid fa-table-list"></i>
                </span>
                <h5 class="fw-bold mb-0 text-dark fs-6">سجل الإثباتات المرفوضة</h5>
            </div>
            <div class="small text-muted">
                إجمالي النتائج: <span class="fw-bold text-indigo">{{ $proofs->total() }}</span> إثبات
            </div>
        </div>
                        ...
                        <tr class="proof-row">
                            ...
                            <td data-label="المسؤول المراجع">
                                <span class="badge bg-light text-secondary border px-2.5 py-1 rounded-pill fw-medium small">
                                    <i class="fa-solid fa-user-shield me-1 text-indigo"></i>
                            ...
                            <td data-label="المرفق">
                                @if(!empty($proof->proof_path))
                                    ...
                                    <a href="{{ asset($proof->proof_path) }}" target="_blank" class="btn btn-sm btn-light border rounded-pill px-2.5 py-1 text-dark small d-inline-flex align-items-center gap-1 shadow-none" title="معاينة الملف المرفق">
                                        <i class="fa-solid {{ $isImg ? 'fa-image text-indigo' : 'fa-paperclip text-secondary' }}"></i>
                                        <span>عرض</span>
                                    </a>
                                @else
                            ...
                            <td class="text-center" data-label="الإجراءات">
                                <div class="d-flex align-items-center gap-1.5 justify-content-center flex-wrap">
                                    @if (Route::has('supplier.payments_proofs_refused.show'))
                                        <a href="{{ route('supplier.payments_proofs_refused.show', $proof->id) }}" class="btn btn-sm btn-indigo text-white rounded-3 shadow-sm px-3 fw-semibold d-inline-flex align-items-center gap-1.5">
                                            <i class="fa-solid fa-eye"></i>
                                            <span>التفاصيل</span>
                                        </a>
                                    @endif

                                    @if (Route::has('supplier.proofs.refused.chat.index'))
                                        <a href="{{ route('supplier.proofs.refused.chat.index', $proof->id) }}" class="btn btn-sm btn-outline-indigo rounded-3 shadow-sm px-2.5" title="محادثة الإدارة حول الرفض">
