        ...
        <div class="row align-items-center position-relative z-1">
            <div class="col-lg-8 mb-3 mb-lg-0">
                <div
                    ...
                    <span class="fw-semibold">{{ __('مزامنة طلبات وبيانات التوريد التلقائية') }}</span>
                    <span class="opacity-50">|</span>
                    <span class="opacity-90">{{ now()->locale('ar')->translatedFormat('l، j F Y') }}</span>
                </div>
                <h1 class="display-6 fw-bold mb-2 text-white text-start">
                    إعدادات جوجل شيت Google Sheets للتوريد 📊
                </h1>
                <p class="text-white-50 mb-0 leading-relaxed fs-6">
                    ربط متجر التوريد الخاص بك بجداول بيانات Google Sheets لمزامنة الطلبات وتحديث البيانات تلقائيًا وبسهولة.
                </p>
            </div>
            <div class="col-lg-4 text-lg-end">
                <div class="d-flex flex-wrap gap-2 justify-content-lg-end">
                    ...
                    <button
                        ...
                        @if ($plan_id == 1 || ($plan_id == 2 && $apps_count >= 1) || ($plan_id == 3 && $apps_count >= 1)) disabled @endif>
        ...
        <div class="col-6 col-md-3">
            <div class="card border-0 shadow-sm rounded-4 h-100 bg-white hover-lift transition-all">
                <div class="card-body p-3.5">
                    ...
                    <h3 class="fw-bold mb-1 text-dark fs-4">{{ $sheets->count() }}</h3>
                    <p class="text-muted small mb-0 fw-semibold">إجمالي الجداول المضافة</p>
                </div>
            </div>
        </div>
        ...
        <div class="col-6 col-md-3">
            <div class="card border-0 shadow-sm rounded-4 h-100 bg-white hover-lift transition-all">
                <div class="card-body p-3.5">
                    ...
                    <h3 class="fw-bold mb-1 text-dark fs-4">{{ $sheets->where('status', 'active')->count() }}</h3>
                    <p class="text-muted small mb-0 fw-semibold">جداول البيانات المفعلة</p>
                </div>
            </div>
        </div>
        ...
        <div class="col-6 col-md-3">
            <div class="card border-0 shadow-sm rounded-4 h-100 bg-white hover-lift transition-all">
                <div class="card-body p-3.5">
                    ...
                    <h3 class="fw-bold mb-1 text-dark fs-4">{{ $sheets->where('status', '!=', 'active')->count() }}</h3>
                    <p class="text-muted small mb-0 fw-semibold">جداول البيانات المعطلة</p>
                </div>
            </div>
        </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4 bg-white">
        <div
            ...
            <div class="d-flex flex-wrap align-items-center gap-2">
                ...
                <h5 class="fw-bold mb-0 text-dark fs-6">قائمة إعدادات Google Sheets للتوريد</h5>
            </div>
            <button
        ...
        <div class="card-body p-0 p-md-3">
            <div class="table-responsive google-sheet-table-responsive custom-table-scroll">
                <table class="table table-hover align-middle mb-0 google-sheet-table">
                    <thead class="bg-light-subtle text-secondary small text-uppercase fw-bold border-bottom">
                        <tr>
                            ...
                            <th class="text-nowrap">معرف Google Sheet</th>
                    ...
                    <tbody>
                        @foreach ($sheets as $index => $sheet)
                            <tr id="row-{{ $sheet->id }}">
                                ...
                                <td data-label="معرف Google Sheet">
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <span class="fw-bold text-dark font-monospace fs-6">
                                            {{ json_decode($sheet->data)->sheet_id }}
                                        </span>
                                    </div>
                                </td>
                                <td data-label="الحالة" class="text-center">
                                    <span
                                        class="badge {{ $sheet->status === 'active' ? 'bg-success-subtle text-success border border-success' : 'bg-secondary-subtle text-secondary border border-secondary' }} rounded-pill px-3 py-1.5 fw-semibold">
        ...
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header text-white py-3.5 px-4"
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <h5 class="modal-title fw-bold mb-0 fs-6 text-white" id="addSheetModalLabel">إضافة إعدادات Google Sheets</h5>
                </div>
            ...
            <div class="modal-body p-4 bg-white">
                <form id="AddGoogleSheetForm">
                    ...
                    <div class="mb-3.5">
                        <label class="form-label fw-bold text-dark small">معرف Google Sheet (Deployment ID) <span
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="sheet_id" id="sheet_id"
                                ...
                                placeholder="أدخل Deployment ID">
                        </div>
                        <div class="invalid-feedback d-block mt-1 small" id="error-sheet_id"></div>
                    </div>

                    <div class="p-3 bg-light rounded-3 border mb-4">
                        <div class="form-check form-switch d-flex align-items-center justify-content-between p-0 mb-0">
                            <label class="form-check-label fw-bold text-dark small mb-0" for="status">تفعيل الجدول تلقائيًا</label>
        ...
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header text-white py-3.5 px-4"
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <h5 class="modal-title fw-bold mb-0 fs-6 text-white" id="editSheetModalLabel">تعديل إعدادات Google Sheets</h5>
                </div>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                    aria-label="إغلاق"></button>
            </div>
            <div class="modal-body p-4 bg-white">
                <form id="EditGoogleSheetForm">
                    ...
                    <div class="mb-3.5">
                        <label class="form-label fw-bold text-dark small">معرف Google Sheet <span
                                class="text-danger">*</span></label>
                        <div class="input-group">
                            <span class="input-group-text bg-light border-end-0 rounded-start-3 text-muted"><i
                                    class="fa-solid fa-key"></i></span>
                            <input type="text" name="sheet_id" id="edit_sheet_id"
                                class="form-control rounded-end-3 shadow-none border-start-0"
                                placeholder="أدخل Sheet ID">
                        </div>
                        <div class="invalid-feedback d-block mt-1 small" id="error-sheet_id"></div>
                    </div>
                    ...
                    <div class="p-3 bg-light rounded-3 border mb-4">
                        <div class="form-check form-switch d-flex align-items-center justify-content-between p-0 mb-0">
                            <label class="form-check-label fw-bold text-dark small mb-0" for="edit_status">تفعيل الجدول</label>
                            <input class="form-check-input ms-0 shadow-none" type="checkbox" name="status"
...
<style>
    ...
    @media (max-width: 1024.98px) {
        ...
        .google-sheet-table-responsive table.google-sheet-table,
        .google-sheet-table-responsive table.google-sheet-table tbody,
        .google-sheet-table-responsive table.google-sheet-table tr,
        .google-sheet-table-responsive table.google-sheet-table td,
        .google-sheet-table-responsive table.google-sheet-table th {
            display: block !important;
            width: 100% !important;
            box-sizing: border-box !important;
        }

        .google-sheet-table-responsive table.google-sheet-table thead {
            display: none !important;
        }

        .google-sheet-table-responsive table.google-sheet-table tbody tr {
        ...
        .google-sheet-table-responsive table.google-sheet-table tbody tr:hover {
            box-shadow: 0 8px 20px rgba(0, 0, 0, 0.08) !important;
        }

        .google-sheet-table-responsive table.google-sheet-table tbody td,
        .google-sheet-table-responsive table.google-sheet-table tbody th {
        ...
        .google-sheet-table-responsive table.google-sheet-table tbody td:last-child {
            border-bottom: none !important;
        }

        .google-sheet-table-responsive table.google-sheet-table tbody td::before,
        .google-sheet-table-responsive table.google-sheet-table tbody th::before {
    ...
    @media (min-width: 768px) and (max-width: 1024.98px) {
        ...
        .google-sheet-table-responsive table.google-sheet-table tbody th[data-label="#"] {
            grid-column: 1 / -1;
            border-bottom: 1px solid #e2e8f0 !important;
            padding-bottom: 0.5rem !important;
        }

        .google-sheet-table-responsive table.google-sheet-table tbody td[data-label="الإجراء"] {
            grid-column: 1 / -1;
            border-bottom: none !important;
