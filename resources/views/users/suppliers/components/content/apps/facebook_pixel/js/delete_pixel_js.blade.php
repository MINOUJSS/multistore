    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        document.querySelectorAll(".delete-btn").forEach(button => {
            button.addEventListener("click", function () {
                let pixelId = this.getAttribute("data-id"); // جلب ID السجل
                let row = document.getElementById(`row-${pixelId}`); // استهداف الصف في الجدول
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        fetch(`{{ url('supplier-panel/apps/facebook-pixel/delete') }}/${pixelId}`, {
                            method: "DELETE",
                            headers: {
