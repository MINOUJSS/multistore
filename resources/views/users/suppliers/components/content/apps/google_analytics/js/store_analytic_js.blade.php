<script>
    ...
    document.getElementById("AddGoogleAnalyticsForm").addEventListener("submit", function (e) {
        e.preventDefault();
        ...
        fetch("{{ route('supplier.app.store-google-analytics') }}", {
            method: "POST",
            body: formData,
            headers: {
        ...
        .then(data => {
            if (data.success) {
                 // 🔹 Get the last index number and increment it
                 let table = document.querySelector("tbody");
                let lastIndex = table.querySelectorAll("tr").length; // Count existing rows
                let newIndex = lastIndex + 1; // Increment for new row
                // ✅ تحديث الجدول دون إعادة تحميل الصفحة
                let newRow = `
                    <tr id="row-${data.data.id}">
                        <td>${newIndex}</td>
                        <td>${JSON.parse(data.data.data).tracking_id}</td>
