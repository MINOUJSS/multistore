<script>
    ...
    document.querySelectorAll(".delete-btn").forEach(button => {
        button.addEventListener("click", function () {
            let analyticsId = this.getAttribute("data-id"); // جلب ID السجل
            let row = document.getElementById(`row-${analyticsId}`); // استهداف الصف في الجدول
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`{{ url('supplier-panel/apps/google-analytics/delete') }}/${analyticsId}`, {
                        method: "DELETE",
                        headers: {
