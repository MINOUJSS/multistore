<script>
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        let badge = checkbox.closest(".integration-card").find(
                            ".status-badge");
                        if (status === "active") {
                            badge.text("مفعل").removeClass("bg-danger").addClass(
                                "bg-success");
                        } else {
                            badge.text("غير مفعل").removeClass("bg-success").addClass(
                                "bg-danger");
                        }
                ...
                error: function(xhr) {
                    ...
                    checkbox.prop("checked", !checkbox.prop(
                        "checked")); // إعادة الحالة الأصلية في حال فشل الطلب
                },
                complete: function() {
                    checkbox.prop("disabled",
                        false); // إعادة تفعيل الـ checkbox بعد انتهاء الطلب
                }
            });
        });

        //--------
        $("#YALIDINEModal").on("show.bs.modal", function() {
        ...
        //------------
        $("#YALIDINEForm").submit(function(event) {
        ...
        //--------------------
        $("#testYalidineConnection").on("click", function() {
            // let wilaya = $("#yl-wilaya").val();
            // let api_id = $("#yl-api_id").val();
            // let api_token = $("#yl-api_token").val();

            // let form = $(this);
            let formData = $("#YALIDINEForm").serialize();

            $.ajax({
                url: "{{ route('supplier.courier-dz.testCredentials') }}", // رابط المسار
                // method: "POST",
                // data: {
                //     wilaya: wilaya,
                //     api_id: api_id,
                ...
                success: function(response) {
                    ...
                    if (response) {
                        Swal.fire({
                            icon: "success",
                            title: "تم التحقق!",
                            text: "تم التحقق من الاتصال بنجاح.",
                            timer: 2000,
                            showConfirmButton: false
                        });
                    }else{
                        Swal.fire({
                            icon: "error",
                            title: "خطاء!",
                            text: "انت لست متصل بالشركة,تحقق من المعلومات المدخلة.!",
                        });
                    }
                },
                error: function(xhr) {
