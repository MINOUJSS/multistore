      ...
      <div class="card-body">
        ...
        <form id="regForm" action="{{route('supplier.payment.redirect')}}" method="POST">
          ...
          <h1 class="text-center">تأكيد الإشتراك</h1>         
            ...
            <div class="tab" style="display:block">إختر الخطة المناسبة لك:
              ...
              @foreach ($plans as $plan)
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="plan" id="flexRadioDefault1" value="{{$plan->id}}" @if($plan->name == get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->name) checked @endif onclick="get_selected_plan();remiz_a_zero_sub_plan_id();get_plan_authorizations({{$plan->id}})">
            ...
            <div id="plan_pricing">
              <div id="plan-pricing-step" class="tab">نوع الإشتراك:
                <div class="mt-3" id ="pricing-details">
                  <div class="form-check">
                    <input class="form-check-input" type="radio" name="plan_price" id="flexRadioDefault1" data-sub-plan-id="0" value="{{get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->price}}<sup>د.ج</sup>/شهر" checked onclick="print_plan_price();">
                    <input type="hidden" name="pre_sub_plan_id" value="0">
                    {{-- <input class="form-check-input" type="radio" name="sub_plan_id" id="flexRadioDefault1" value="{{get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->id}}<sup>د.ج</sup>/شهر" checked onclick="print_plan_price();"> --}}                    <label class="form-check-label" for="flexRadioDefault1">
                      <label class="form-check-label" for="flexRadioDefault1">
                      {{ get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->price}}<sup>د.ج</sup>/30 يوم
                    </label>
                  </div>
                  @foreach (get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->pricing as $index => $price)
                    <div class="form-check">
                      <input class="form-check-input" type="radio" name="plan_price" id="flexRadioDefault1" data-sub-plan-id="{{$price->id}}" value="{{$price->price}}<sup>د.ج</sup>/{{ $price->duration}} يوم" @if($price->duration == get_supplier_data(Auth::user()->tenant->id)->plan_subscription->duration) checked @endif onclick="print_plan_price();">
                      <input type="hidden" name="pre_sub_plan_id" value="{{$price->id}}">
                      {{-- <input type="hidden" name="sub_plan_id" value="{{get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->id}}"> --}}
                      <label class="form-check-label" for="flexRadioDefault1">
            ...
            <div class="mt-3" id="plan-name"><b>الخطة:{{get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->name}}</b></div>
            <div id="plan-price">{{get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->price}}<sup>د.ج</sup>/{{get_supplier_data(Auth::user()->tenant->id)->plan_subscription->duration}} يوم</div>
