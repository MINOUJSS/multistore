        ...
        <div class="row align-items-center position-relative z-1">
            <div class="col-lg-8 mb-3 mb-lg-0">
                <div
                    ...
                    <span class="fw-semibold">{{ __('تأكيد وتفعيل خطة التوريد') }}</span>
                    <span class="opacity-50">|</span>
                    <span class="opacity-90">{{ now()->locale('ar')->translatedFormat('l، j F Y') }}</span>
                </div>
                <h1 class="display-6 fw-bold mb-2 text-white text-start">
                    تأكيد الإشتراك للتوريد 💳
                </h1>
                <p class="text-white-50 mb-0 leading-relaxed fs-6">
                    اختر الخطة ومدة الاشتراك المناسبة ووسيلة الدفع المفضلة لتأكيد وتجديد حساب التوريد الخاص بك.
                </p>
            </div>
                ...
                <form id="regForm" action="{{ route('supplier.payment.redirect') }}" method="POST">
                    ...
                    <div class="tab" style="display:block">
                        <div class="d-flex align-items-center gap-2 mb-3">
                            <span class="avatar avatar-sm rounded-circle bg-navy-subtle text-navy fw-bold me-1"
                                    ...
                                    <div class="option-card p-3.5 rounded-3 border bg-light-subtle transition-all h-100 position-relative">
                                        <div class="form-check d-flex align-items-center gap-3">
                                            <input class="form-check-input flex-shrink-0 fs-5 mt-0" type="radio" name="plan" id="plan_radio_{{ $plan->id }}"
                                                value="{{ $plan->id }}"
                                                @if($plan->name == get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->name) checked @endif
                                    ...
                                    <div class="option-card p-3.5 rounded-3 border bg-light-subtle transition-all h-100">
                                        <div class="form-check d-flex align-items-center gap-3">
                                            <input class="form-check-input flex-shrink-0 fs-5 mt-0" type="radio" name="plan_price" id="pricing_default"
                                                data-sub-plan-id="0"
                                                value="{{ get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->price }}<sup>د.ج</sup>/شهر"
                                            ...
                                            <label class="form-check-label w-100 cursor-pointer text-start" for="pricing_default">
                                                ...
                                                <span class="text-navy fw-bold fs-5">
                                                    {{ number_format(get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->price, 2) }} <sup>د.ج</sup>
                                                </span>
                                        ...
                                        <div class="option-card p-3.5 rounded-3 border bg-light-subtle transition-all h-100">
                                            <div class="form-check d-flex align-items-center gap-3">
                                                <input class="form-check-input flex-shrink-0 fs-5 mt-0" type="radio" name="plan_price" id="pricing_opt_{{ $price->id }}"
                                                    ...
                                                    @if($price->duration == get_supplier_data(Auth::user()->tenant->id)->plan_subscription->duration) checked @endif
                                ...
                                <div class="option-card p-3.5 rounded-3 border bg-light-subtle transition-all">
                                    <div class="form-check d-flex align-items-center gap-3">
                                        ...
                                        <label class="form-check-label w-100 cursor-pointer text-start" for="pay_method_cib">
                                            ...
                                            <span class="text-muted small">تسديد فوري وتفعيل تلقائي لحساب التوريد عبر بوابة ChargilyPay</span>
                                        </label>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div id="subscription_info" class="tab" style="display:none">
                        ...
                        <div class="bg-light-subtle rounded-4 p-4 border text-start">
                            <div class="mb-3 border-bottom pb-2" id="plan-name">
                                <b>الخطة: {{ get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->name }}</b>
                            </div>
                            <div class="mb-2 text-navy fs-5 fw-bold border-bottom pb-2" id="plan-price">
                                {{ get_supplier_plan_data(get_supplier_data(Auth::user()->tenant->id)->plan_subscription->plan_id)->price }}<sup>د.ج</sup>/{{ get_supplier_data(Auth::user()->tenant->id)->plan_subscription->duration }} يوم
                            </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
                        ...
                        <button class="btn btn-supplier-primary fw-bold px-4 py-2.5 rounded-3 ms-auto" type="button" id="nextBtn" onclick="nextPrev(1)">
