<div class="container-fluid py-3 px-3 px-md-4">
        ...
        <div class="row align-items-center position-relative z-1">
            <div class="col-lg-8 mb-3 mb-lg-0">
                <div
                    ...
                    <span class="fw-semibold">{{ __('إدارة عروض الأقسام والتوريد') }}</span>
                    <span class="opacity-50">|</span>
                    <span class="opacity-90">{{ now()->locale('ar')->translatedFormat('l، j F Y') }}</span>
                </div>
                <h1 class="display-6 fw-bold mb-2 text-white text-start">
                    ربط كوبونات التوريد بالتصنيفات والأقسام 🏷️
                </h1>
                <p class="text-white-50 mb-0 leading-relaxed fs-6">
                    تخصيص وتحديد كوبونات التوريد لأقسام وتصنيفات محددة، متابعة التخفيضات المخصصة وإدارة العلاقات بسهولة وسرعة.
                </p>
            </div>
        ...
        <div class="col-6 col-md-4">
            <div class="card border-0 shadow-sm rounded-4 h-100 bg-white hover-lift transition-all">
                <div class="card-body p-3.5">
                    ...
                    <h3 class="fw-bold mb-1 text-dark fs-4">{{ count($linkedCategories) }}</h3>
                    <p class="text-muted small mb-0 fw-semibold">إجمالي الكوبونات المرتبطة بأقسام</p>
                </div>
            </div>
        </div>
        ...
        <div class="col-6 col-md-4">
            <div class="card border-0 shadow-sm rounded-4 h-100 bg-white hover-lift transition-all">
                <div class="card-body p-3.5">
                    ...
                    <p class="text-muted small mb-0 fw-semibold">كوبونات التوريد المتاحة</p>
                </div>
            </div>
        </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 h-100 bg-white hover-lift transition-all">
                <div class="card-body p-3.5">
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        ...
                        <span class="badge bg-success-subtle text-success rounded-pill small">الأصناف</span>
                    </div>
                    <h3 class="fw-bold mb-1 text-dark fs-4">{{ count($categories) }}</h3>
                    <p class="text-muted small mb-0 fw-semibold">التصنيفات المتاحة للتخصيص</p>
                </div>
            </div>
        </div>
    </div>

    <!-- 🔹 اختيار الكوبون والقسم Form Section -->
    <div class="card border-0 shadow-sm rounded-4 mb-4 bg-white" id="linkFormSection">
        <div class="card-header bg-white border-0 py-3.5 px-3 px-md-4 d-flex align-items-center gap-2 border-bottom border-light">
            ...
            <h5 class="fw-bold mb-0 text-dark fs-6">تخصيص وربط كوبون بقسم للتوريد</h5>
        </div>
        <div class="card-body p-3.5 p-md-4">
            <form action="{{ route('supplier.categories-coupons.store') }}" method="POST">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <label for="coupon_id" class="form-label fw-semibold text-secondary small">
                            <i class="fa-solid fa-ticket me-1 text-navy"></i> اختر الكوبون
                        </label>
                    ...
                    <div class="col-md-6">
                        <label for="category_id" class="form-label fw-semibold text-secondary small">
                            <i class="fa-solid fa-tags me-1 text-navy"></i> اختر القسم
                        </label>
                        <select name="category_id" id="category_id" class="form-select rounded-3 border-light-subtle shadow-none" required>
                    ...
                    <div class="col-12 mt-4 text-end">
                        <button type="submit" class="btn btn-supplier-primary rounded-3 px-4 py-2.5 fw-bold shadow-sm d-inline-flex align-items-center gap-2">
                            <i class="fa fa-link"></i>
                            <span>ربط الكوبون بالقسم المحدد</span>
                        </button>
                    </div>
                </div>
            </form>
    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4 bg-white">
        <div class="card-header bg-white border-0 py-3.5 px-3 px-md-4 d-flex flex-wrap align-items-center justify-content-between gap-2 border-bottom border-light">
            <div class="d-flex flex-wrap align-items-center gap-2">
                ...
                <h5 class="fw-bold mb-0 text-dark fs-6">جدول الأقسام المربوطة حالياً للتوريد</h5>
            </div>
            <span class="badge bg-light text-dark border px-3 py-1.5 rounded-pill small fw-semibold">
                <i class="fa-solid fa-layer-group me-1 text-navy"></i> إجمالي الروابط: {{ count($linkedCategories) }}
            </span>
        </div>
        <div class="card-body p-0 p-md-3">
            <div class="table-responsive categories-coupons-table-responsive custom-table-scroll">
                <table class="table table-hover align-middle mb-0 categories-coupons-table">
                    <thead class="bg-light-subtle text-secondary small text-uppercase fw-bold border-bottom">
                        <tr>
                            ...
                            <th class="text-nowrap">الكوبون</th>
                            <th class="text-nowrap">القسم</th>
                            <th class="text-nowrap">تاريخ الإنشاء</th>
                            <th class="pe-4 text-end text-nowrap">إجراءات</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($linkedCategories as $link)
                            <tr>
                                <td data-label="#" class="ps-md-4">
                                    <span class="fw-bold text-secondary">{{ $loop->iteration }}</span>
                                </td>
                                <td data-label="الكوبون">
                                    <span class="badge bg-navy-subtle text-navy fw-bold font-monospace px-3 py-1.5 fs-6 rounded-3 border border-navy-subtle">
                                        <i class="fa-solid fa-ticket me-1"></i>{{ $link->coupon->code }}
                                    </span>
                                </td>
                                <td data-label="القسم">
                                    <span class="fw-bold text-dark fs-6">{{ $link->category->name }}</span>
                                </td>
                                <td data-label="تاريخ الإنشاء">
                                    <span class="text-secondary small fw-medium"><i class="fa-regular fa-clock me-1 text-muted"></i>{{ $link->created_at->format('Y-m-d H:i') }}</span>
                                </td>
                                <td data-label="إجراءات" class="pe-md-4 text-end">
                                    <form action="{{ route('supplier.categories-coupons.destroy', $link->id) }}" method="POST" class="d-inline delete-form" onsubmit="return confirm('هل أنت متأكد من حذف هذا الربط؟');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center text-muted py-5 fw-semibold">
                                    ...
                                    لا توجد روابط بين الأقسام والكوبونات بعد.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
<style>
    ...
    @media (max-width: 1024.98px) {
        .categories-coupons-table-responsive table.categories-coupons-table,
        .categories-coupons-table-responsive table.categories-coupons-table tbody,
        .categories-coupons-table-responsive table.categories-coupons-table tr,
        .categories-coupons-table-responsive table.categories-coupons-table td {
            display: block !important;
            width: 100% !important;
            box-sizing: border-box !important;
        }

        .categories-coupons-table-responsive table.categories-coupons-table thead {
            display: none !important;
        }

        .categories-coupons-table-responsive table.categories-coupons-table tbody tr {
        ...
        .categories-coupons-table-responsive table.categories-coupons-table tbody tr:hover {
            box-shadow: 0 8px 20px rgba(0, 0, 0, 0.08) !important;
        }

        .categories-coupons-table-responsive table.categories-coupons-table tbody td {
        ...
        .categories-coupons-table-responsive table.categories-coupons-table tbody td:last-child {
        ...
        .categories-coupons-table-responsive table.categories-coupons-table tbody td::before {
    ...
    @media (min-width: 768px) and (max-width: 1024.98px) {
        ...
        .categories-coupons-table-responsive table.categories-coupons-table tbody td[data-label="#"] {
            grid-column: 1 / -1;
            border-bottom: 1px solid #e2e8f0 !important;
            padding-bottom: 0.5rem !important;
        }

        .categories-coupons-table-responsive table.categories-coupons-table tbody td[data-label="إجراءات"] {
            grid-column: 1 / -1;
            border-bottom: none !important;
            justify-content: flex-end !important;
        }
    ...
    @media (min-width: 1025px) {
        .categories-coupons-table th {
            font-size: 0.82rem;
            letter-spacing: 0.3px;
            padding: 0.9rem 0.75rem;
        }

        .categories-coupons-table td {
