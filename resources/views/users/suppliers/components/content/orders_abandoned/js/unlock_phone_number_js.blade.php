    ...
    function unlock_phone_number(order_id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/supplier-panel/order-abandoned/unlock-phone-number/${order_id}`,
                    method: "POST",
                    headers: {
                        ...
                        if (response.status == 'error') {
                            Swal.fire({
                                ...
                                footer: response.message ===
                                    'رصيدك غير كاف لفتح هذا الرقم،عليك بتعبئت رصيدك أولاً.' ?
                                    '<a href="/supplier-panel/wallet">شحن الرصيد</a>' :
                                    undefined,
