<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.edit-global-category', function() {
            ...
            $.get(`/supplier-panel/page/section/global/categories/${id}/edit`, (data) => {
        ...
        $(document).on('click', '.edit-store-category', function() {
            const id = $(this).data('id');
            $.get(`/supplier-panel/page/section/store/categories/${id}/edit`, (data) => {
        ...
        $('#addGlobalCategoryForm').submit(handleSubmit('POST',
            '/supplier-panel/page/section/global/categories'));
        $('#editGlobalCategoryForm').submit(handleSubmit('PUT', (form) => {
            const id = $('#edit_global_category_id').val();
            return `/supplier-panel/page/section/global/categories/${id}/update`;
        }));

        $('#addStoreCategoryForm').submit(handleSubmit('POST', '/supplier-panel/page/section/store/categories',
            true));
        $('#editStoreCategoryForm').submit(handleSubmit('PUT', (form) => {
            const id = $('#edit_store_category_id').val();
            return `/supplier-panel/page/section/store/categories/${id}/update`;
        }, true));
            ...
            return function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `/supplier-panel/page/section/${type}/categories/${id}/delete`,
                            type: 'DELETE',
                            data: {
