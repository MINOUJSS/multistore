    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            ...
            <div class="modal-body p-4 bg-white text-start">
                ...
                @php
                    $supplierCommercialBanks = get_admin_bank_accounts('bank');
                @endphp
                @forelse($supplierCommercialBanks as $bankAcc)
                    @include('users.suppliers.components.content.payments.inc.admin_account_card', ['account' => $bankAcc])
                @empty
                    @include('users.suppliers.components.content.payments.inc.admin_account_card', ['type' => 'bank'])
                @endforelse

                <form action="{{ route('supplier.wallet.recharge.ccp') }}" method="POST" enctype="multipart/form-data" class="mt-3 pt-3 border-top">
                    @csrf
