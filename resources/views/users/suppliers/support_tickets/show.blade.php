        ...
        <div class="row align-items-center position-relative z-1">
            <div class="col-lg-8 mb-3 mb-lg-0">
                <div
                    ...
                    <span class="fw-semibold">{{ __('تفاصيل المحادثة والدعم المباشر للموردين') }}</span>
            ...
            <div class="col-lg-4 text-lg-end d-flex flex-wrap align-items-center justify-content-lg-end gap-2">
                @if($ticket->status !== 'closed')
                    <form action="{{ route('supplier.support_tickets.close', $ticket->id) }}" method="POST" onsubmit="return confirm('هل أنت تأكد من إغلاق هذه التذكرة؟')">
                        @csrf
            ...
            <div class="col-6 col-md-3 border-end-md">
                ...
                <span class="fw-bold text-dark fs-6">
                    @switch($ticket->category)
                        @case('technical') <i class="fa-solid fa-code text-primary me-1"></i> تقني / جملة @break
                        @case('financial') <i class="fa-solid fa-coins text-success me-1"></i> مالي وسحوبات @break
                        @case('shipping') <i class="fa-solid fa-truck text-warning me-1"></i> الشحن وإدارة الطلبات @break
    ...
    <div class="card border-0 shadow-sm rounded-4 bg-white mb-4 overflow-hidden">
        <div class="card-header bg-white py-3 px-4 border-bottom">
            <h5 class="fw-bold mb-0 text-dark d-flex align-items-center gap-2">
                <i class="fa-solid fa-comments text-navy"></i>
        ...
        <div class="card-body p-4 bg-light-subtle">
            ...
            <div class="d-flex flex-column align-items-start mb-4">
                <div class="d-flex align-items-center gap-2 mb-2">
                    <span class="badge bg-navy-subtle text-navy rounded-circle p-2 d-inline-flex align-items-center justify-content-center" style="width: 36px; height: 36px;">
                        <i class="fa-solid fa-user fs-6"></i>
                    </span>
                    <div>
                        <strong class="text-dark small d-block">{{ $ticket->user_name }} (المورد)</strong>
                ...
                <div class="p-4 bg-white border rounded-4 shadow-sm max-w-2xl text-dark leading-relaxed ms-md-4 w-100">
                    <h6 class="fw-bold text-navy mb-2 border-bottom pb-2">{{ $ticket->subject }}</h6>
                        ...
                        <div class="mt-3 pt-3 border-top">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                @foreach($ticket->attachments as $attachment)
                                    <a href="{{ asset($attachment) }}" target="_blank" class="btn btn-sm btn-outline-navy rounded-3 small fw-semibold">
            ...
            @foreach($ticket->replies as $reply)
                @if($reply->sender_type === 'user')
                    <div class="d-flex flex-column align-items-start mb-4">
                        <div class="d-flex align-items-center gap-2 mb-2">
                            ...
                            <div>
                                <strong class="text-dark small d-block">{{ $reply->sender_name }} (المورد)</strong>
                ...
                @else
                    <div class="d-flex flex-column align-items-end mb-4 text-start">
                        <div class="d-flex align-items-center gap-2 mb-2">
                            <div class="text-end">
                                <strong class="text-navy small d-block">فريق الكول سنتر والدعم الفني 🎧</strong>
                                <small class="text-muted fs-7">{{ $reply->created_at->diffForHumans() }}</small>
                            </div>
                            <span class="badge bg-supplier-primary text-white rounded-circle p-2 d-inline-flex align-items-center justify-content-center shadow-xs" style="width: 36px; height: 36px;">
                                <i class="fa-solid fa-headset fs-6"></i>
                            </span>
                        </div>
                        <div class="p-4 text-white rounded-4 shadow-sm max-w-2xl leading-relaxed me-md-4 w-100" style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%);">
                            {!! nl2br(e($reply->message)) !!}
        ...
        @if($ticket->status !== 'closed')
            <div class="card-footer bg-white p-4 border-top">
                <h6 class="fw-bold text-dark mb-3"><i class="fa-solid fa-reply me-1 text-navy"></i>إضافة رد جديد:</h6>
                <form action="{{ route('supplier.support_tickets.reply', $ticket->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <textarea name="message" rows="4" class="form-control rounded-3 p-3" placeholder="اكتب ردك وملاحظاتك المباشرة هنا لمشرف الدعم..." required></textarea>
                    </div>
                    ...
                    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3">
                        ...
                        <button type="submit" class="btn btn-supplier-primary px-5 py-2.5 rounded-3 fw-bold shadow-sm w-100 w-sm-auto">
