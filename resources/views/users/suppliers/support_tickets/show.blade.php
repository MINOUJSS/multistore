...
<div class="container-fluid px-4 py-3">
    ...
    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3 mb-4">
        <div>
            <div class="d-flex align-items-center gap-2 mb-1">
                <a href="{{ route('supplier.support_tickets.index') }}" class="btn btn-sm btn-outline-secondary rounded-circle">
                    <i class="fa-solid fa-arrow-right"></i>
                </a>
                <h3 class="fw-bold mb-0 text-dark">
                    تذكرة رقم <span class="dir-ltr text-indigo">#{{ $ticket->ticket_number }}</span>
                </h3>
            </div>
        ...
        <div class="d-flex align-items-center gap-2">
            ...
            @endif
            <a href="{{ route('supplier.support_tickets.index') }}" class="btn btn-outline-secondary btn-sm rounded-3 px-3">
                العودة للتذاكر
            </a>
        </div>
    </div>
            ...
            <div class="col-6 col-md-3 border-end border-light">
                ...
                <span class="fw-bold text-dark mt-1 d-inline-block">
                    @switch($ticket->category)
                        @case('technical') تقني / جملة @break
                        @case('financial') مالي وسحوبات @break
                        @case('shipping') الشحن وإدارة الطلبات @break
                        @default استفسار عام
                    @endswitch
                </span>
            </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 bg-white mb-4">
        <div class="card-header bg-white py-3 px-4 border-0">
            <h5 class="fw-bold mb-0 text-dark"><i class="fa-solid fa-comments me-2 text-indigo"></i>سجل المحادثة والردود</h5>
        </div>
        ...
        <div class="card-body p-4 bg-light bg-opacity-50">
            ...
            <div class="d-flex flex-column align-items-start mb-4">
                <div class="d-flex align-items-center gap-2 mb-1">
                    ...
                    <strong class="text-dark small">{{ $ticket->user_name }} (المورد)</strong>
                ...
                <div class="p-3 bg-white border border-light-subtle rounded-3 shadow-xs max-w-2xl text-dark leading-relaxed ms-4">
                    <h6 class="fw-bold text-indigo mb-2">{{ $ticket->subject }}</h6>
                ...
                @if($reply->sender_type === 'user')
                    <div class="d-flex flex-column align-items-start mb-4">
                        <div class="d-flex align-items-center gap-2 mb-1">
                            ...
                            <strong class="text-dark small">{{ $reply->sender_name }} (المورد)</strong>
                ...
                @else
                    <div class="d-flex flex-column align-items-end mb-4">
                        <div class="d-flex align-items-center gap-2 mb-1">
                            ...
                            <strong class="text-indigo small">مشرف الدعم والكول سنتر 🎧</strong>
                            <span class="badge text-white rounded-circle p-2" style="background-color: #4f46e5; width: 32px; height: 32px; display: inline-flex; align-items: center; justify-content: center;">
                                <i class="fa-solid fa-headset"></i>
                            </span>
                        </div>
                        <div class="p-3 text-white rounded-3 shadow-xs max-w-2xl leading-relaxed me-4" style="background-color: #4f46e5;">
                            {!! nl2br(e($reply->message)) !!}
        ...
        @if($ticket->status !== 'closed')
            <div class="card-footer bg-white p-4 border-top">
                <h6 class="fw-bold text-dark mb-3"><i class="fa-solid fa-reply me-1 text-indigo"></i>إضافة رد جديد:</h6>
                <form action="{{ route('supplier.support_tickets.reply', $ticket->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <textarea name="message" rows="4" class="form-control rounded-3" placeholder="اكتب ردك وملاحظاتك المباشرة هنا لمشرف الدعم..." required></textarea>
                    </div>
                    ...
                    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3">
                        ...
                        <button type="submit" class="btn text-white px-4 fw-bold rounded-3" style="background-color: #4f46e5;">
        ...
        @else
            <div class="card-footer bg-light p-3 text-center text-muted border-top">
                <i class="fa-solid fa-lock me-1"></i> هذه التذكرة مغلقة. يمكنك إنشاء تذكرة جديدة لأي استفسار آخر.
            </div>
        @endif
    </div>
