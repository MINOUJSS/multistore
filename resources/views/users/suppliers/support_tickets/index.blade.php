...
<div class="container-fluid px-4 py-3">
    ...
    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3 mb-4">
        <div>
            <h3 class="fw-bold mb-1 text-dark">
                <i class="fa-solid fa-headset text-indigo me-2"></i>تذاكر الدعم الفني والكول سنتر للموردين
            </h3>
            <p class="text-muted small mb-0">متابعة الاستفسارات الخاصة بتوزيع المنتجات للجملة، المالية، ومتابعة ردود الإدارة.</p>
        </div>
        <div>
            <button type="button" class="btn text-white shadow-sm px-4 fw-bold rounded-3" style="background-color: #4f46e5;" data-bs-toggle="modal" data-bs-target="#newSupplierTicketModal">
    ...
    <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden">
        <div class="card-header bg-white py-3 px-4 border-0 d-flex flex-wrap align-items-center justify-content-between gap-3">
            <h5 class="fw-bold mb-0 text-dark"><i class="fa-solid fa-list-check me-2 text-indigo"></i>سجل التذاكر</h5>
            <div class="d-flex align-items-center gap-2">
                <a href="{{ route('supplier.support_tickets.index') }}" class="btn btn-sm btn-outline-secondary rounded-pill px-3 {{ !request()->filled('status') ? 'active' : '' }}">الكل</a>
                <a href="{{ route('supplier.support_tickets.index', ['status' => 'open']) }}" class="btn btn-sm btn-outline-warning rounded-pill px-3 {{ request('status') === 'open' ? 'active' : '' }}">مفتوحة</a>
                <a href="{{ route('supplier.support_tickets.index', ['status' => 'answered']) }}" class="btn btn-sm btn-outline-success rounded-pill px-3 {{ request('status') === 'answered' ? 'active' : '' }}">تم الرد</a>
                <a href="{{ route('supplier.support_tickets.index', ['status' => 'closed']) }}" class="btn btn-sm btn-outline-dark rounded-pill px-3 {{ request('status') === 'closed' ? 'active' : '' }}">مغلقة</a>
            </div>
        </div>
                        ...
                        @forelse($tickets as $ticket)
                            <tr>
                                ...
                                <td class="text-end pe-4">
                                    <a href="{{ route('supplier.support_tickets.show', $ticket->id) }}" class="btn btn-sm btn-outline-indigo rounded-3 px-3">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-5 text-muted">
                                    ...
                                    <h6>لا يوجد تذاكر دعم فني للموردين حالياً</h6>
                                    <p class="small text-muted mb-3">يمكنك إنشاء تذكرة وسيقوم الفريق التقني المخصص للموردين بالمتابعة معك.</p>
                                    <button type="button" class="btn btn-sm text-white rounded-3 px-4" style="background-color: #4f46e5;" data-bs-toggle="modal" data-bs-target="#newSupplierTicketModal">
                                        أنشئ تذكرتك الأولى
                                    </button>
                                </td>
                            </tr>
...
<div class="modal fade" id="newSupplierTicketModal" tabindex="-1" aria-labelledby="newSupplierTicketModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header text-white border-0 py-3" style="background: linear-gradient(135deg, #4f46e5 0%, #3730a3 100%);">
            ...
            <form action="{{ route('supplier.support_tickets.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-footer bg-light border-0 px-4 py-3">
                    ...
                    <button type="submit" class="btn text-white rounded-3 px-4 fw-bold" style="background-color: #4f46e5;">
