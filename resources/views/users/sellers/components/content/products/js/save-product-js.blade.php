<script>
    ...
    $('#save_product').on('click', function(e) {
        ...
        // if (isValid) {
        $.ajax({
            url: '/seller-panel/product/create',
            type: 'POST',
            data: formData,
    ...
    //--------------------------------------------------------
    // عند الضغط على زر إضافة فيديو جديد
    $('#add_add_video').on('click', function() {
        var container = document.getElementById("add_product_video");
    ...
    // عند تغيير نوع الفيديو (يوتيوب / محلي)
    $(document).on('change', '.video-type-select', function() {
        var parent = $(this).closest('.video_container');
        var type = $(this).val();

        if (type === 'youtube' || type === 'vimeo') {
            parent.find('.youtube-input').removeClass('d-none');
            parent.find('.local-input').addClass('d-none');
            parent.find('input[name="videos[file][]"]').val('');
        } else {
            parent.find('.youtube-input').addClass('d-none');
            parent.find('.local-input').removeClass('d-none');
            parent.find('input[name="videos[youtube_url][]"]').val('');
        }
    });
    ...
    function add_get_product_attribute(user_id) {
        ...
        $.ajax({
            url: '/seller-panel/attributes/user/' + user_id,
            type: 'POST',
            success: function(response) {
    ...
    function add_add_discount() {
        ...
        var filesHtml =
            '<div id="add_discount_container" class="add_discount_container border position-relative p-3 mt-3 mb-3 row">' +
            '<div class="col-3">' +
            ...
            '<input name="add_discount_start_date" type="date" class="form-control discount-required" value="' +
            ...
            '<input name="add_discount_end_date" type="date" class="form-control discount-required" value="' +
            new Date().toISOString().split('T')[0] + '">' +
