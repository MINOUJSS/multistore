            ...
            <div class="card shadow border-0 rounded-4">
                <div class="card-body p-4 p-md-5">
                    ...
                    <div class="bg-light border rounded-3 p-3 p-md-4 mb-4">
                        <h4 class="mb-1 fw-semibold">الخطة: {{ get_seller_plan_data($order->plan_id)->name }}</h4>
                        <p class="text-muted small mb-2">{{get_seller_plan_data($order->plan_id)->description}}</p>
                        <h5 class="text-success">{{ $plan_price }} <sup>د.ج</sup> / {{ $subscriptionDuration }} يوم</h5>

                        <ul class="list-unstyled mt-3">
                            @foreach (get_seller_plan_data($order->plan_id)->authorizations as $item)
                    ...
                    @php
                        ...
                        //     else
                        //     {
                        //         $total=get_rest_off_current_seller_plan($old_subscription->seller_id, $old_subscription->plan_id, $order->plan_id, $rest_days) ;
                        //     }
                    ...
                    <form action="{{route('seller.subscription.paymethod.redirect')}}" method="POST">
                        ...
                        <div class="mb-4">
                            @if ($order->price<=0)
                                <p class="text-success fw-bold text-center">بالضغط على زر تأكيد الطلب يتم الإنتقال إلى الخطة  {{ get_seller_plan_data($order->plan_id)->name }} و شحن الرصيد بـ: {{ abs(get_plan_price_from_id_and_duration($order->plan_id, $order->duration) - $old_subscription->price) }} د.ج </p>
