...
<div class="container-fluid py-3 px-3 px-md-4">
    @php
        $sellerData = get_seller_data(auth()->user()->tenant_id);
        $currentPlanId = $sellerData->plan_subscription->plan_id ?? null;
        ...
        <div class="row align-items-center position-relative z-1">
            <div class="col-lg-8 mb-3 mb-lg-0">
                <div
                    ...
                    <span class="fw-semibold">{{ __('خطط الاشتراكات والترقية') }}</span>
                    <span class="opacity-50">|</span>
                    <span class="opacity-90">{{ now()->locale('ar')->translatedFormat('l، j F Y') }}</span>
                </div>
                <h1 class="display-6 fw-bold mb-2 text-white text-start">
                    اشتراك الباقة والترقية 💳
                </h1>
                <p class="text-white-50 mb-0 leading-relaxed fs-6">
                    اختر الباقة المناسبة لطموح متجرك واستمتع بجميع المميزات المتقدمة للتجارة الإلكترونية لتكبير مبيعاتك.
                </p>
            </div>
    ...
    @if (is_seller_has_plan_order(get_seller_data(auth()->user()->tenant_id)->id) &&
            get_seller_data(auth()->user()->tenant_id)->orderPlan[0]->payment_method != null)
