...
<div class="container mt-4">
    ...
    <div class="card">
        <div class="card-body">
                ...
                {{-- {{dd(get_seller_plan_data(get_seller_data(auth()->user()->tenant_id)->orderPlan[0]->plan_id)->name)}} --}}
                <div>
            ...
            <div class="row mt-3">
                @foreach ($plans as $plan)
                    @php
                        ...
                        $isActive = $plan->id == get_seller_subscription_data(get_seller_data(auth()->user()->tenant_id)->id)->plan_id; // تحديد الباقة الأولى كـ "مفعلة"
                    ...
                    <div class="col-md-4 col-sm-6 mb-3">
                        <div class="card shadow-sm {{ $isActive ? 'active-plan' : '' }}">
                            <div class="card-header text-center bg-primary text-white">
                                ...
                                <div style="{{$plan->id==get_seller_data(auth()->user()->tenant_id)->plan_subscription->plan_id || get_seller_data(auth()->user()->tenant_id)->plan_subscription->plan_id==3? 'display:none;' : ''}}">
                                    <hr>
                                    <h4>عروض الخطة</h4>
                                    <ul class="list-group">
                            ...
                            <div class="card-body">
                                ...
                                {{-- <form action="{{route('seller.subscription.order.plan',$plan->id)}}" method="POST" class="text-center">
                                    @csrf --}}
                                    <button type="submit" class="btn btn-success w-100" {{ $isActive ? 'Disabled' : '' }} {{$plan->id==1 || $plan->id==get_seller_data(auth()->user()->tenant_id)->plan_subscription->plan_id || get_seller_data(auth()->user()->tenant_id)->plan_subscription->plan_id==3? 'hidden' : ''}}>قم بالترقية الآن</button>
                                </form>
                            </div>
                        </div>
                    </div>
