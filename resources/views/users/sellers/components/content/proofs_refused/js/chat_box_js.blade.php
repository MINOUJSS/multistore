...
<script>
    ...
    openChatBtn.addEventListener('click', async () => {
        ...
        // Mark messages as read (from seller's perspective)
        try {
            await fetch(`{{ route('seller.proofs.refused.chat.read', ['proofId' => $proof->id]) }}`, { // Updated route
                method: "POST",
                headers: {
        ...
        try {
            const res = await fetch(
                `{{ route('seller.proofs.refused.chat.get_messages', ['proofId' => $proof->id]) }}`, { // Updated route
                    method: "GET",
                    headers: {
                        "X-CSRF-TOKEN": csrfToken
                    }
    ...
    // Send message
    document.getElementById('sellerChatForm').addEventListener('submit', async function(e) { // Changed form ID
        e.preventDefault();
        ...
        try {
            const res = await fetch(
                "{{ route('seller.proofs.refused.chat.send', ['proofId' => $proof->id]) }}", { // Updated route
                    method: "POST",
                    body: formData
                });
            if (res.ok) {
    ...
    async function fetchMessages() {
        try {
            const res = await fetch(
                `{{ route('seller.proofs.refused.chat.fetch', ['proofId' => $proof->id]) }}`, { // Updated route
                    method: "GET",
                    headers: {
                        "X-CSRF-TOKEN": csrfToken
                    }
    ...
    function appendMessage(text, sender_type, date, attachments = []) { // Renamed 'sender' to 'sender_type'
        ...
        // 'admin' (backend) means the admin is chatting with the seller -> display as 'seller' (left)
        // 'seller' (backend) means the seller is chatting -> display as 'admin' (right)
        if (sender_type === 'admin') {
            uiRole = 'admin'; // Display as seller (left side)
