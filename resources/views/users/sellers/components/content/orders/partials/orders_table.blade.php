@forelse($orders as $order)
    <tr class="{!! get_order_status_class($order->status) !!}">
        ...
        <td data-label="رقم الهاتف">{!! seller_order_display_phone($order->id) !!}</td>
        ...
        <td>
            <select class="form-select form-select-sm order-status" data-order-id="{{ $order->id }}" data-label=" تأكيد الطلب">
