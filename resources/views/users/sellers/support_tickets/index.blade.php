        ...
        <div class="row align-items-center position-relative z-1">
            <div class="col-lg-8 mb-3 mb-lg-0">
                <div
                    ...
                    <span class="fw-semibold">{{ __('مركز الخدمة والكول سنتر') }}</span>
                    <span class="opacity-50">|</span>
                    <span class="opacity-90">{{ now()->locale('ar')->translatedFormat('l، j F Y') }}</span>
                </div>
                <h1 class="display-6 fw-bold mb-2 text-white text-start">
                    تذاكر الدعم الفني والكول سنتر 🎧
                </h1>
                <p class="text-white-50 mb-0 leading-relaxed fs-6">
                    متابعة جميع استفساراتك، وتقديم طلبات المساعدة الفنية، أو مراجعة المحادثات والتذاكر السابقة بسهولة.
                </p>
            </div>
            <div class="col-lg-4 text-lg-end">
                <button type="button"
                    ...
                    <i class="fa-solid fa-plus-circle text-plum"></i>
        ...
        <div class="col-12 col-md-4">
            <div class="card border-0 shadow-sm rounded-4 p-4 bg-white transition-all hover-lift">
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <div class="stat-icon-wrapper bg-plum-subtle text-plum p-3 rounded-3">
    ...
    <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden mb-4">
        <div class="card-header bg-white py-3 px-4 border-bottom d-flex flex-wrap align-items-center justify-content-between gap-3">
            <h5 class="fw-bold mb-0 text-dark d-flex align-items-center gap-2">
                <i class="fa-solid fa-list-check text-plum"></i>
                <span>سجل التذاكر والمحادثات</span>
            </h5>
            <div class="d-flex align-items-center gap-1.5 flex-wrap">
                <a href="{{ route('seller.support_tickets.index') }}"
                    class="btn btn-sm rounded-pill px-3 py-1.5 fw-bold transition-all {{ !request()->filled('status') ? 'btn-seller-primary' : 'btn-light text-dark' }}">
                    الكل
                </a>
                <a href="{{ route('seller.support_tickets.index', ['status' => 'open']) }}"
                ...
                <a href="{{ route('seller.support_tickets.index', ['status' => 'answered']) }}"
                ...
                <a href="{{ route('seller.support_tickets.index', ['status' => 'closed']) }}"
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0 seller-tickets-table">
                        ...
                        @forelse($tickets as $ticket)
                            <tr>
                                <td class="ps-4" data-label="رقم التذكرة">
                                    <span class="fw-bold text-plum dir-ltr">#{{ $ticket->ticket_number }}</span>
                                </td>
                                ...
                                <td class="text-end pe-4" data-label="الإجراءات">
                                    <a href="{{ route('seller.support_tickets.show', $ticket->id) }}"
                                        class="btn btn-sm btn-outline-plum rounded-3 px-3 fw-semibold">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-5 text-muted">
                                    <div class="mb-3 display-4 text-plum opacity-50">🎫</div>
                                    <h6 class="fw-bold text-dark mb-1">لا يوجد تذاكر دعم فني حالياً</h6>
                                    <p class="small text-muted mb-3">يمكنك إنشاء تذكرة جديدة وسيقوم فريق الكول سنتر والدعم الفني بمساعدتك فوراً.</p>
                                    <button type="button" class="btn btn-seller-primary rounded-3 px-4 py-2 fw-bold"
                                        data-bs-toggle="modal" data-bs-target="#newTicketModal">
        ...
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            ...
            <form action="{{ route('seller.support_tickets.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-footer bg-light-subtle border-top px-4 py-3">
                    ...
                    <button type="submit" class="btn btn-seller-primary rounded-3 px-4 py-2 fw-bold">
...
<style>
    ...
    @media (max-width: 1024.98px) {
        .seller-tickets-table thead {
            display: none;
        }

        .seller-tickets-table,
        .seller-tickets-table tbody,
        .seller-tickets-table tr,
        .seller-tickets-table td {
            display: block;
            width: 100%;
        }
        ...
        .seller-tickets-table td:last-child {
            border-bottom: none;
            padding-top: 0.75rem !important;
            justify-content: flex-end;
        }

        .seller-tickets-table td::before {
        ...
        .seller-tickets-table td.ps-4 {
            padding-left: 0 !important;
        }

        .seller-tickets-table td.pe-4 {
