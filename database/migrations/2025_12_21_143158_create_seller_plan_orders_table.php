...
return new class extends Migration {
    ...
    {
        Schema::create('seller_plan_orders', function (Blueprint $table) {
            $table->bigIncrements('id'); // المفتاح الأساسي
            $table->unsignedBigInteger('plan_id'); // المفتاح الخارجي للخطة
            $table->unsignedBigInteger('seller_id'); // المفتاح الخارجي للمورد
            ...
            $table->foreign('seller_id')->references('id')->on('sellers')->onDelete('cascade'); // علاقة مع جدول الموردين
            $table->foreign('plan_id')->references('id')->on('seller_plans')->onDelete('cascade'); // علاقة مع جدول الخطط
        });
    }
    ...
    {
        Schema::dropIfExists('seller_plan_orders');
    }
};
